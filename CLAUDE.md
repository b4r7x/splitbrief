...
- **Runner selection** is the `src/features/runners/` feature; its picker entry is the page `src/app/overlays/runners.tsx`. One `ToolModelPicker` serves four roles (`SeatPickerRole`: planner · implementer · reviewer · escalation) — do not fork a per-role picker. `ToolModelPicker` is a component name, not a `tool-picker` folder boundary.
...
- **Sizing** — every overlay panel, the home body and the home/summary composer take their width from `overlayRect` (`src/core/navigation/overlay-rect.ts`, densities `compact`/`roomy`/`wide`); `OverlayPanel` has no `maxWidth`. The workflow composer, the transcript and the full-bleed `editor` overlay stay full-width (see the workflow bullet below).
