...
const SHIM_FIXTURE = join(import.meta.dirname, '../../fixtures/opencode/opencode-shim.cjs');

function writeOpencodeShim(opts: { dir: string; capture: string }): string {
  const shimPath = join(opts.dir, 'opencode');
  copyFileSync(SHIM_FIXTURE, shimPath);
  writeFileSync(
    join(opts.dir, 'opencode-shim.json'),
    JSON.stringify({
      capture: opts.capture,
      research: RESEARCH,
      spec: SPEC,
      plan: planWithFiles(8),
      sentinelA: SENTINEL_A,
      sentinelB: SENTINEL_B,
    }),
    'utf8',
  );
