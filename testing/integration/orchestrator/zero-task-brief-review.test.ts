...
function makePlannerWithInvalidTaskRepair(): {
  ...
  prepared.planner.plan = async (opts) => {
    ...
    return {
      ...result,
      phases: [
        { filename: 'spec.md', text: result.spec },
        { filename: 'plan.md', text: result.plan },
        { filename: 'tasks.md', text: 'The planner returned no Task Brief.' },
      ],
    };
  };
...
describe('zero-task planning fails before brief review', { timeout: 30_000 }, () => {
  it.each([
    'standard',
    'speckit',
  ] as const)('%s records the quality failure without entering the blank brief-review screen', async (mode) => {
    ...
    expect(planner.repairCallCount()).toBe(1);
    ...
    expect(events.some((event) => event.phase === 'reviewing-briefs')).toBe(false);
    ...
    if (mode === 'speckit') {
      expect(events.some((event) => event.phase === 'analyzing')).toBe(false);
    }
  });
});
