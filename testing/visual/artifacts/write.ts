...
import { tryResolveElementLocator, type ResolvedElementLocator } from '../locators.js';
...
export async function writeArtifactBundle(
  ...
  try {
    ...
    assertUniqueArtifactPaths(run.artifacts.map((artifact) => artifact.record.files));
    ...
    const manifest = buildManifest({
      ...
      renderer: artifactRecords.some((artifact) => artifact.files.svg !== null)
...
async function buildRunArtifacts(options: {
  readonly captures: readonly VisualFrameCapture[];
  readonly selection: CaptureSelection;
...
async function prepareWritableArtifactPath(file: StagedArtifactFile): Promise<string> {
...
async function verifyStagedArtifact(options: {
  readonly root: string;
  readonly artifact: PendingArtifact;
}): Promise<void> {
  for (const file of options.artifact.files) {
