...
> Entries below the `[Unreleased]` section are a dated historical record and are kept
> verbatim. They name commands, config keys and documents that have since been removed;
> the current surface is what the docs under `docs/` describe.
...
- Crew is no longer a surface of its own: it is the first section of Settings, one labelled row per seat (`PLAN`, `BUILD`, `REVIEW`), and the same rows build the first-run Setup screen. The Settings seat sections that showed the same three facts in a second vocabulary are gone; ready-made crews are offered in Setup only, where choosing one is still a single save.
...
- Reasoning effort is chosen with the model in the seat picker and mirrored read-only on every seat row, and it no longer rewires an inherited reviewer as a side effect.
...
- Models that differ only by reasoning effort, speed, or thinking budget are one row in the tool/model picker instead of one row per combination. The row expands into a tree of one child per axis the family varies along — `effort`, `fast`, `thinking` — and an expanded parent drops its summary chip and keeps the disclosure glyph, because the children spell out what the chip summarized. `space` cycles the axis under the cursor in place, `⏎` confirms the drafted variant from any row of the family — parent or child, so choosing never means walking back up — and a mouse click on an axis row steps it rather than confirming and dismissing the picker, an axis with nowhere left to step staying an ordinary choice. Only a closed set of option tokens peels off an id (`src/features/runners/model-catalog/option-axis.ts`), so a model carrying any other suffix keeps a row of its own.
...
- The runtime command registry is 23 commands in five categories (Navigate, Crew, Workflow, View, Input & output), no command carrying an alias, and, where a command takes an argument, prefill. `/crew [seat]` opens Crew focused on that seat's row (PLAN when no seat is given). No row in the palette errors on Enter.
...
- A quota halt offers one row per tool the blocked seat can move to — `switch BUILD to OpenCode CLI`, keyed `[w1]`/`[w2]` — instead of one row listing several behind a single key, and the key pressed is the tool applied. Its headline is one line that fits an 80-column panel (`recovery needed · Ollama hit its limit · resets 17:00`), and the reset is spelled by one formatter — local 24-hour clock, dated only when it is not today — on the panel and on the workflow header, which marks the affected seat while the halt is open. A recovery panel too short for its prompt now keeps its headline and every action row instead of scrolling the choices off screen.
- The stderr effort-drop warning (`src/engine/runners/factory-gates.ts`) is kept for headless runs; in the TUI the picker now clears an unsupported effort at save time and says so in the feedback row.
- **Breaking.** `approval.headless` no longer exists, and a run that has a reply channel is now always prompted. The key only ever forced fail-closed on a run that could have asked — a `--json` or `--plain` run with nowhere to send the question already fails closed on its own — so the behaviour it bought is the default, and `ApprovalConfigSchema` is strict, which makes a stale `approval.headless: true` a load error rather than a silently dropped key that would quietly reopen prompting.
...
- One name per fact on every surface. A model's name is prettified by one formatter (`formatToolModel` and `formatModelName`, `src/core/model-display.ts`), which the summary, the transcript and the workflow activity header all read, so a raw id never reaches a rendered cell. A seat's billing posture is one word from `billingWord` (`src/core/runners/runner-billing.ts`). `PLAN`, `BUILD` and `REVIEW` are the only seat words, from `CREW_SEAT_LABELS` (`src/core/crew/identity.ts`) — on home, Settings ∋ Crew, Setup, the workflow activity header, the cost drilldown and the summary, which gains a `REVIEW` row of its own. An inherited review seat reads as one sentence, `same as planner` (`PLANNER_INHERITANCE`, `src/core/crew/identity.ts`), with `= planner` as its mark.

### Removed

- The commands `/effort`, `/repomap`, `/resume`, `/attach`, and `/detach`. Effort is chosen in the seat picker (`/crew <seat>` is the way in), the repo map rebuilds itself on every planning run, a paused workflow resumes from its own approval prompt, and images are attached with `/image`. Each name still answers for one release with a pointer at its replacement (`REMOVED_COMMANDS`, `src/core/runtime/commands/types.ts`) rather than `Unknown command`.
...
**Breaking — subsystems.** Each of these is gone in full: its modules, its CLI subcommand or runtime command, its config keys, its tests and its documentation. A retired CLI name prints what replaced it instead of starting a workflow (`RETIRED_COMMANDS`, `src/cli/unknown-command.ts`).

- **Brief recovery.** The parallel brief/state authority that re-derived a run's briefs after an interrupted planning phase, with its schemas and its `T-065` invariant gate. A run that cannot finish planning is resumed with `splitbrief continue <id>` or re-planned.
- **The detached-run IPC layer.** `splitbrief attach`, `detach`, `ps`, the `--detach` flag, the attach socket and the IPC prompt dispatcher. Runs are foreground; `splitbrief status` reports one, `splitbrief continue <id>` re-enters it. The session lock file and the run heartbeat survive as `src/core/sessions/lockfile.ts` and `src/engine/orchestrator/run/heartbeat.ts`.
- **The RPC server.** `src/cli/rpc/` and the `--rpc` flag. `--json` still emits the same NDJSON event stream, and `--plain` is the new one-line-per-phase text stream.
- **The transcript-persistence policy.** The transcript, `session.jsonl` and the review packet are always written; there is no policy module, no switch and no `safety.transcriptSafe` marker (its only reader went with the policy). Payload byte-bounding and credential redaction are unchanged, under `src/core/payload-bounds.ts`.
- **The MCP server.** `splitbrief mcp` and `src/engine/mcp/`.
- **The `--worktree` feature.** `splitbrief worktree`, the `--worktree` flag and the `.trees/` checkout root. Per-run isolation is a different thing and stays (`src/engine/orchestrator/isolation/worktree.ts`); a pre-existing `.trees/` is still kept out of file discovery, git status and the evidence.
- **`splitbrief explain`** and the explain renderer.
- **`splitbrief handoff`**, the `/handoff` runtime command and `trust.customRenderers`. A run's artifacts are read from `.splitbrief/sessions/<id>/`.
- **The planner estimate-review gate** and its prompt. The brief-approval gate (`workflow.approve`) is the remaining stop before implementation.
- **Automatic splitting of an overflowing task.** An overflowing task is reported; re-plan or narrow the feature description.
- **The session tree.** `session-tree.jsonl`, `tree-meta.json` and the tree recorder sink.
- **The OpenTelemetry sink**, `--otel-exporter`, and the `@opentelemetry/api` and `@opentelemetry/sdk-trace-base` dependencies.
- **The HTML session export.** `splitbrief export` and `/export`.
- **Cumulative statistics.** `splitbrief stats`, `status --history`, and the savings figures that promised them.
- **`splitbrief snapshot` and the automatic snapshot triggers.** Snapshots themselves stay as the baseline for change detection and `/run reject`; nothing schedules them from config any more.
- **Module hooks.** The `kind: module` hook variant, the built-in hook bundle (`hooks.builtin`) and the `pre_compact` event. A hook is an executable and its argv; transcript compaction is automatic and fires no hook.

**Breaking — config keys.** Fifteen config shapes are gone — twelve keys, plus three values of keys that still exist — and there is no version bump: `version: 3` is still the only accepted version, so a stale config is corrected by hand. Every one of them is a **load error**, and one load names every offending path at once, one line per path, so a config carrying several is corrected in a single pass — a key in a strict block (`workflow`, `hooks`, `codebase`, `approval`, `approval.tiers`) and a removed top-level key alike, even though the root is otherwise not strict. `docs/CONFIGURATION.md` §16 **Removed config keys** is the migration table.

- Rejected at load, nested: `workflow.persistTranscript`, `workflow.briefReview: rich` (only `simple` parses), `hooks.builtin`, `hooks.pre_compact`, a hook entry's `kind: module` (only `command` parses), `approval.headless`, `approval.tiers.network`, `approval.tiers.validation`, `codebase.cacheDir`, `sessions.scope: global` (only `project` parses).
- Rejected at load, top-level: the whole `otel` block, the whole `snapshots` block (including `snapshots.auto.*`), the whole `trust` block, `plannerEstimateReview`, `autoSplitOverflow`.

**Breaking — runtime commands.** Ten names left the registry, which is why it is 23 commands and there is no alias mechanism left: `/compact-transcript` (compaction is automatic), `/export` and `/handoff` (read `.splitbrief/sessions/<id>/`), `/yolo` (approval is turned off with `approval.enabled: false`, one home for the value), and the six aliases `/config` → `/settings`, `/planner` → `/crew plan`, `/implementer` → `/crew build`, `/reviewer` → `/crew review`, `/accept-run` → `/run accept`, `/reject-run` → `/run reject`. Each prints its pointer for one release.

### Fixed

- The Cursor Agent CLI subscription `session` channel works on macOS, where the credential is a login-keychain item the file bridge cannot carry. `hostKeychainPlatforms` now includes that channel, and a channel that resolves to `host-account` there keeps the host `HOME` and `USER` — the two values the keychain's search list and item account are keyed on — while `TMPDIR`, `XDG_*`, `npm_config_cache`, `PIP_CACHE_DIR` and `CARGO_HOME` stay redirected into `.splitbrief/sandbox/` and nothing is copied to disk. Linux and Windows still file-bridge. The security cost of that channel — `~`-relative reads and Cursor Agent CLI's own state writes land in the real home, and the login keychain resolves by default — is stated in `docs/API-KEYS.md`.
- The inline (non-fullscreen) TUI no longer repaints a row low. Ink 6.8's incremental writer assumes the cursor rests on the last rendered line, but Ink writes a frame shorter than the terminal with a trailing newline and the cursor a row below it, so every repaint of a short frame landed off by one — visible on the inline path and on the fallback a failed fullscreen init drops into. `incrementalRendering` is dropped from that path (`src/cli/render/app.ts`), at the stated cost of a whole-frame rewrite per repaint, which Ink brackets in synchronized output on a TTY and leaves unbracketed on a pipe; under CI it costs nothing, since Ink drops every non-static redraw there anyway. Ink 7.0.0 fixes the arithmetic, so the option can come back behind an `ink >= 7.0.0` bump the manifest's `^6.8.0` pin has not taken. `testing/integration/cli/render-path-repaint.test.ts` replays the frames through a headless terminal and holds the line.
...
- **OpenTelemetry sink** — opt-in (`otel.enabled: true`) span emission for workflow lifecycle, phases, and tasks. Per-cost attributes. Users register their own exporter. (`docs/OTEL.md`, since removed)
