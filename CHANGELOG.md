...
- Crew is no longer a surface of its own: it is the first section of Settings, one labelled row per seat (`PLAN`, `BUILD`, `REVIEW`) with an effort row under each, and the same rows build the first-run Setup screen. The Settings seat sections that showed the same three facts in a second vocabulary are gone; ready-made crews are offered in Setup only, where choosing one is still a single save.
