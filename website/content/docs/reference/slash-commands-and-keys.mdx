---
title: Slash commands & keys
description: Complete reference for every slash command and keyboard shortcut available in the SPLITBRIEF TUI.
---
...
If you only want a short overview, see [`FEATURES.md`](https://github.com/b4r7x/splitbrief/blob/main/docs/FEATURES.md#slash-commands-palette). If you are adding a new command, follow the contract in [`src/core/runtime/commands/types.ts`](https://github.com/b4r7x/splitbrief/blob/main/src/core/runtime/commands/types.ts) and the dispatch rules in [`src/core/runtime/commands/dispatch.ts`](https://github.com/b4r7x/splitbrief/blob/main/src/core/runtime/commands/dispatch.ts).
...
- **Args**: none.
- **Example**: `/refresh`
- **Behavior**: Immediately prints `"Refreshing tool detection…"` (with an ellipsis glyph) and requests a project-file refresh. On success it prints `"Tool detection refreshed"`. If detection throws, the feedback line shows the actual error text via `toErrorMessage(error)`; it does not replace the error with a generic `"Tool detection failed"` message. Detection runs asynchronously, and picker overlays read from the refreshed cache when next opened.
...
- **Behavior**: Opens the `settings` overlay. Mutations are persisted to project config on save. In an attached workflow client, `/settings` is not exposed and `Ctrl+,` is explicitly ignored so a local-only config change cannot be mistaken for a detached-server change.
- **Implementation**: catalog at `src/core/runtime/commands/registry.ts`; opens overlay via `overlayStore.open`.
...
- **Purpose**: Navigate back to the home screen from the workflow or summary screen. Leaving a locally owned active workflow unmounts the workflow UI and aborts that local run. In an attached client, it only leaves the viewer; the detached server keeps running the workflow.
- **Screens**: `workflow`, `summary`.
- **Args**: none.
- **Example**: `/home`
- **Behavior**: Calls `routerStore.navigate({ to: 'home' })`. The home screen shows the prompt input and recent sessions. When the current workflow is attached, unmount cleanup closes the viewer's IPC socket without stopping the detached server process.
...
- **Purpose**: Exit the local TUI immediately. For a locally owned run, teardown kills in-flight subprocesses through the process registry. For an attached client, only the local viewer exits; the detached server and its workflow continue.
- **Screens**: all.
- **Args**: none.
- **Shortcut**: `Ctrl+Q` (`src/core/keybindings/registry.ts`). On a locally owned workflow screen, `Ctrl+C` twice within 2 seconds also exits. An attached client or non-workflow screen exits on the first `Ctrl+C`.
- **Example**: `/quit`
- **Behavior**: Calls the `exit` callback wired in `src/app/command-context.ts`. No confirmation prompt. An attached client closes its local IPC connection as the TUI exits; that connection teardown does not terminate the detached server run. Use `/home` if you want to leave the workflow screen without exiting the local TUI.
...
| `Ctrl+,` | Open settings overlay; ignored on an attached workflow client | `src/app/keys.ts` |
| `Ctrl+Q` | Quit application | `src/app/keys.ts` |
| `Ctrl+C` | Interrupt in-flight turn (workflow screen) or exit otherwise | `src/app/keys.ts` |
| `Ctrl+C` ×2 | Exit a locally owned workflow (second press while exit is armed, within 2s); attached clients and non-workflow screens exit on the first press | `src/app/keys.ts` |
...
macOS terminals do not always forward physical `PageUp`, `PageDown`, `Home`, or `End` without an `Fn` layer. In normal workflow input, the composer owns text-editing chords such as `Ctrl+B`, `Ctrl+E`, and `Ctrl+F`; `Ctrl+E` moves the cursor to the end of the current line, so `/scroll` is the reliable command path for conversation paging.
...
Review commands and shortcut:

| Command | Action |
|---|---|
| `approve` / `y` | Approve briefs |
| `Ctrl+E` | Open the built-in inline editor. In `reviewing-spec` / `reviewing-plan`, it opens the full-surface editor over `spec.md` / `plan.md`; in `reviewing-briefs`, it opens the field editor over the focused Task Brief. |
| `e` / `edit` | Open persisted `tasks.md` in the external editor. `VISUAL` is explicit; otherwise SPLITBRIEF uses non-terminal `EDITOR`, detected GUI editors from safe absolute `PATH` segments, macOS `open -W -t`, terminal `EDITOR`, and finally `vi`; Windows detection honors `PATHEXT` plus `.cmd`, `.exe`, and `.bat` shims. |
...
`Ctrl+E` during a review phase opens the built-in inline editor: in `reviewing-spec` / `reviewing-plan` it opens a full-surface overlay over `spec.md` / `plan.md`; in `reviewing-briefs` it opens the small-viewport field editor over the focused Task Brief. Task Brief review has a single shortcut owner. Spec and plan review currently also register a legacy external-editor shortcut on the same chord, so use the typed `edit-file` command when you explicitly want the external editor. Once an inline editor is open, it owns keyboard input. `$EDITOR` remains the opt-in path for bulk or structural edits.
...
- [`FEATURES.md`](https://github.com/b4r7x/splitbrief/blob/main/docs/FEATURES.md#slash-commands-palette) — the shorter overview document, useful as a quick refresher.
- [`docs/WORKFLOW.md`](/docs/concepts/workflow-modes-and-phases) — workflow phases, modes, and the rewind state machine.
- [`docs/CONCEPTS.md`](/docs/concepts/glossary) — shared vocabulary (phase, queue, awaiting-continue, etc.).
- [`docs/ARCHITECTURE.md`](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) — how the TUI, stores, engine, and runners are layered.
- [`docs/STORES.md`](https://github.com/b4r7x/splitbrief/blob/main/docs/STORES.md) — `configStore`, `overlayStore`, `feedbackStore`, `lifecycleStore`, `routerStore`, `attachmentsStore`.
