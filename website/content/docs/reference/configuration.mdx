---
title: Configuration
description: Complete reference for .splitbrief/config.yaml — the single declarative file that wires SPLITBRIEF to your planner, implementer, validation tools, workflow gates, hooks, snapshots, and observability.
---

This document is a field-by-field reference. For end-user mode semantics see [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases); for hook plumbing see [HOOKS-CONFIG.md](/docs/guides/hooks); for repo-map tuning see [REPOMAP.md](/docs/guides/repo-map); for OpenTelemetry export see [OTEL.md](/docs/guides/observability-otel).
...
validation:     { typecheck, lint, test, testCommand, timeoutMs }
...
|---|---|---|---|
| `model` | string | — | Model identifier. The planner schema makes this optional because some CLI and Agent SDK planners supply a default, but an `api` planner needs an operationally resolvable model: omission never selects a catalog default, so set an explicit model or use `auto` for a known provider with a catalog default. Implementer: required for every runner kind; use `auto` when the selected runner has a known default. |
...
| `contextLength` | int > 0 | API planner: `8192`; API implementer: `32768` | Assumed context window used for prompt budgeting. Override it when the provider or self-hosted Ollama/LM Studio endpoint has a different real window; this field does not change the provider-side limit. |
| `temperature` | 0..2 | API planner/implementer: `0.3` | Sampling temperature. Honored only by the `api` kind (planner and implementer); the `cli`, `shell`, `agent`, and `agent-sdk` kinds cannot pass it to their backend and drop it with a stderr warning. |
...
| `effort` | `low\|medium\|high\|xhigh` | unset | For the `api` kind (planner and implementer) maps to Anthropic `thinking.budget_tokens` (2k / 8k / 24k / 48k). The Agent SDK planner forwards it as the SDK's first-class `effort` option. The current Agent SDK implementer accepts the field in config but does not forward it; other runner kinds may ignore it. |
...
| `apiKey` | string | no | Inline key. **Strongly prefer the matching env var** for official provider endpoints (see [API-KEYS.md](/docs/guides/api-key-security)). Use an inline key for known providers with a custom/proxy `apiBase`; env-sourced provider keys are rejected for that case. |
| `model` | non-empty string | operationally yes | The planner schema accepts omission, but the API adapter fails initialization when no model resolves. Set an explicit model. `auto` works only when the provider is known and has a catalog default; it is not a fallback for a custom provider. |
...
| `capabilities` | partial object | no | **Planner only.** Declares optional planner features such as `supportsConversationalPlanning`, `supportsHintEscalation`, and `supportsSelfSummarisation` so the orchestrator skips features the wrapper cannot provide. `supportsSessionResume: true`, `supportsEffort: true`, and `supportsImages: true` are rejected on `shell`/`agent` planners: command-based adapters have no documented session-handle contract and no channel for effort or image attachments. A `false` value is accepted but does not add capability. Ignored (and rejected) on `implementer` — implementer write behavior is set via profile `capabilities.writesFiles`. |
...
**See also:** §3 `implementer`, §11 environment variables, [API-KEYS.md](/docs/guides/api-key-security).
...
**See also:** §2 `planner`, §6 `escalation` (for mid-tier fallback), [REPOMAP.md](/docs/guides/repo-map) (codebase context the implementer never sees, only the planner).
...
validation: {
  ...
  timeoutMs?:        number; // optional, integer >= 1000
}
```
...
| `testPattern` | string | — | Optional **basename template**, not a general glob. For the built-in `npm test` fallback, SPLITBRIEF replaces only the first `*` with the affected source basename and checks that literal filename beside the source and in corresponding `tests/` or `test/` directories. The template must not contain `/`, `\`, or `..`; use at most one `*`. It has no targeting effect when a configured, discovered, or heuristic test command is selected: that command runs as-is. Without a template, the built-in fallback checks the TypeScript forms `<name>.test.ts` and `<name>.test.tsx`. |
| `timeoutMs` | integer >= 1000 | `600000` | Per-stage wall-clock timeout in milliseconds for typecheck, lint, and test commands, including baseline probes. Each stage gets its own timeout budget. |
...
**See also:** [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) (where validation sits in the loop), `src/engine/orchestrator/validation/run.ts`.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `mode` | enum | `standard` | `instant` \| `quick` \| `standard` \| `speckit`. A `version: 2` config with legacy `full` is rewritten to `speckit` during the v2→v3 migration; `full` is rejected in `version: 3` YAML. |
...
| `briefReview` | enum | `simple` | `simple` review. `rich` is deprecated, accepted for compatibility, and treated as `simple`. During review, `Ctrl+E` opens the built-in inline editor; typed `e`, `edit`, `E`, or `edit-file` opens the persisted document in the external editor. |
...
| Mode | Nominal pre-build planner calls | Default `approve` | Default `briefReview` | Auto snapshots | Best for |
|---|:---:|:---:|:---:|:---:|---|
...
| `speckit` | 5–6 | `all` | `simple` | off | Large, risky, externally visible |

`approve: default` resolves to the table above via `resolveApproveLevel()` (`src/core/config/runtime/resolve.ts`). A successful run adds one planner call for final review after implementation.
...
- `briefReview: simple` — the supported brief review mode. Legacy `briefReview: rich` configs are accepted but mapped to `simple`. Use `Ctrl+E` for the built-in inline editor; type `e`, `edit`, `E`, or `edit-file` for the external editor.
...
**See also:** [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) (`/mode`, `/effort` runtime overrides). Workflow approval level is set with `--approve` or `workflow.approve`.

---

## 6. `escalation`

When a task fails locally past `maxRetries`, SPLITBRIEF escalates through a tier ladder: **tier 0** retries with the "intermediate" mid-tier model configured here, then **tier 1** has the planner write a hint, then **tier 2** hands the task to the planner ("full escalation"). The intermediate tier runs only when both `intermediateProvider` and `intermediateModel` are set and `enabled` is not `false`.
...
escalation: {
  ...
  intermediateModel?:     string; // required when intermediateProvider is set
}
```
...
| `intermediateModel` | string | conditionally required | Model id at that provider. It is required when `intermediateProvider` is set; config validation fails if it is missing. There is no implementer-model fallback. |
...
**When to use:** you run a cheap implementer (Ollama / DeepSeek) and want a "10x cheaper than the planner but smarter than the implementer" stop along the way before paying for an Opus retry. Skip if your implementer is already frontier-class. Setting both `intermediateProvider` and `intermediateModel` turns the tier on; add `enabled: false` only when you want to keep that configuration but bypass the tier.

---

## 7. `codebase`

Repo-map context block injected into planner prompts. Detailed semantics: [REPOMAP.md](/docs/guides/repo-map).
...
**See also:** [REPOMAP.md](/docs/guides/repo-map).

---

## 8. `hooks`

Workflow lifecycle hooks. Quick overview here; full schema, trust model, and substitution rules in [HOOKS-CONFIG.md](/docs/guides/hooks).
...
|---|---|---|
| `prettier-on-change` | off | Currently no-ops: it runs on `post_task`, whose `task_completed` event has no `file` field for the built-in to format. Do not rely on this toggle as a formatting gate. |
...
**See also:** [HOOKS-CONFIG.md](/docs/guides/hooks) for trust prompts (`--allow-hooks`), substitution variables, environment passed to hook processes.

---

## 9. `otel`

OpenTelemetry span sink. Full details: [OTEL.md](/docs/guides/observability-otel).
...
**See also:** [OTEL.md](/docs/guides/observability-otel).
...
**See also:** [SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys).
...
Declared in `src/cli/options.ts` for workflow commands (`start`, `resume`, `continue`, `last`) plus command-specific registrations. These flags **override** the matching config field for the current invocation only. [`CLI-REFERENCE.md`](/docs/reference/cli) is the canonical flag matrix.
...
| `--json` | Headless: writes one JSON readiness report first, then NDJSON `{ type: 'event', data: EngineEvent }` envelopes to stdout, no TUI | start, resume, continue, last |
...
- [PRINCIPLES.md](https://github.com/b4r7x/splitbrief/blob/main/docs/PRINCIPLES.md) — one-page rule index
- [ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) — runner contracts, orchestrator loop, event model
- [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) — mode + approval semantics in depth
- [HOOKS-CONFIG.md](/docs/guides/hooks) — hook events, `HookEntry` schema, trust model
- [REPOMAP.md](/docs/guides/repo-map) — `codebase.*` semantics, PageRank, cache
- [OTEL.md](/docs/guides/observability-otel) — span hierarchy, exporter setup
- [API-KEYS.md](/docs/guides/api-key-security) — secret handling and redaction
- [SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) — runtime overrides and palette commands
- [DEBUGGING.md](/docs/guides/debugging-a-run) — diagnosing config load failures
- [ERRORS.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ERRORS.md) — `ConfigError` shape and exit codes
- [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) — workflow modes and approval semantics
