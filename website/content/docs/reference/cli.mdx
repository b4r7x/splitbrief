---
title: CLI
description: Complete reference for every splitbrief command. Maintained against src/cli.ts and src/cli/commands/*.ts.
---
...
- **Exit codes:**
  - `0` — success. A headless workflow interrupted by SIGINT or SIGTERM currently also returns `0` after saving an interrupted session; treat the saved session status, not that exit code alone, as the completion signal.
...
- **Interactive signal exits:** When an OS termination signal reaches the TUI process, it restores terminal state and uses conventional codes: `130` for SIGINT, `143` for SIGTERM, and `129` for SIGHUP.
- **Error format:** Failures print `Error: <message>` in red on stderr. Programmatic callers should grep stderr, not stdout.
- **OpenTelemetry:** `src/cli.ts` calls `bootstrapOtel()` before parsing. When `otel.enabled: true` in config and `--otel-exporter console` is passed (where supported), spans flow to stdout. See [OTEL.md](/docs/guides/observability-otel).
...
```
splitbrief start [feature] [files...] [options]
```
...
**`@file` syntax.** The variadic `[files...]` position also accepts additional feature words. Arguments prefixed with `@` are resolved as file paths; unprefixed arguments are appended to the feature text. Text files are injected into planner context, and image files are queued as planner attachments. Example: `splitbrief "refactor auth" @context.md @screenshot.png`.

### Usage

```
splitbrief start [feature] [files...] [--mode <mode>] [--auto] [--approve <level>] \
...
|---|---|---|---|
| `--mode <mode>` | enum | `standard` (or `workflow.mode` from config) | One of `instant`, `quick`, `standard`, `speckit`. `full` is a legacy alias for `speckit`. See [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases). |
...
| `--no-fullscreen` | boolean | fullscreen on for an interactive TTY | Keep the TUI on the normal screen instead of entering the alternate buffer. Mouse tracking and hover are also disabled because they require fullscreen. |
| `--no-mouse` | boolean | mouse on when fullscreen | Disable Ink mouse tracking while leaving fullscreen enabled. This also disables hover. |
| `--hover` | boolean | `false` | Opt in to hover highlighting. It takes effect only in an interactive TTY with fullscreen and mouse tracking enabled. |
...
- `0` — workflow completed, the TUI exited cleanly, or a headless SIGINT/SIGTERM interruption was saved cleanly. In the signal case the session remains interrupted and must not be treated as completed.
...
- [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md), [CONFIGURATION.md](/docs/reference/configuration).
...
- `--no-fullscreen`, `--no-mouse`, and `--hover` configure only an interactive TUI. They are no-ops for `--json`, `--rpc`, and the background server created by `--detach`; a later `attach` chooses its own TUI settings.
...
- [TASK-CONTRACT.md](/docs/reference/task-brief-format), [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
Bootstrap a project. Creates `.splitbrief/config.yaml` when needed, loads it into the TUI setup screen, and saves the planner / implementer selections you confirm. Re-run with `--reconfigure` to edit an existing config through the same setup flow.
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--reconfigure` | boolean | `false` | Open setup with the existing `.splitbrief/config.yaml` loaded so you can edit it. The flag bypasses the early “already exists” return; it does not delete or reset the file before setup. |
...
# Review or change the current planner and implementer
splitbrief init --reconfigure
```

### Exit codes

- `0` — an existing config was left unchanged, or setup exited cleanly after creating or editing config.
- `1` — TUI render failure or unexpected I/O error.

### Files affected

- **Reads:** existing `.splitbrief/config.yaml`, provider environment.
- **Writes:** `.splitbrief/config.yaml`, store directories under `.splitbrief/`.

### See also

- [CONFIGURATION.md](/docs/reference/configuration) — every config field.
- [API-KEYS.md](/docs/guides/api-key-security) — credential resolution.
- [BOOTSTRAP.md](https://github.com/b4r7x/splitbrief/blob/main/docs/BOOTSTRAP.md) — startup sequence.

### Behavior notes

- If `.splitbrief/config.yaml` already exists and `--reconfigure` is not passed, init prints `Config already exists at .splitbrief/config.yaml` plus `Use --reconfigure to overwrite.` and exits `0`.
- With `--reconfigure`, `initStores()` loads the current config before setup. Confirming choices saves the edited config; the command does not unlink or replace it with defaults on entry.
...
| `--no-fullscreen` / `--no-mouse` / `--hover` | Configure the TUI when the interrupted session resumes interactively. Fullscreen is required for mouse tracking, and mouse tracking is required for hover. They are no-ops with `--json` or `--rpc`. |
...
- `0` — resumed successfully, or a headless resumed run received SIGINT/SIGTERM and saved its interrupted state cleanly.
...
- [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [docs/CHANGELOG.md](/docs/reference/changelog) — schema migrations.

### Behavior notes

- `maybeMigrate(projectDir)` runs before state load, so legacy layouts are upgraded transparently.
- The stale-version error body is:

  ```text
  session '<id>' state is from an older version and cannot be resumed.
...
- Interactive resume resolves the display flags exactly like `start`: fullscreen defaults on only for an interactive TTY, mouse requires fullscreen, and hover requires an explicit `--hover` plus mouse and fullscreen.
...
| Other resume flags | — | — | Runner overrides, mode, budget, OTel, `--no-fullscreen`, `--no-mouse`, `--hover`, and approval controls. The three TUI flags apply on both live attach and interactive interrupted-resume paths. `--worktree` is rejected — it is a `start`-only flag, since a resumed session already lives in its original worktree. |
...
- `0` — attached or resumed successfully. A headless interrupted-resume path also returns `0` when SIGINT/SIGTERM saves the session as interrupted.
...
- When the target session is not running but has resumable state, `continue` delegates to `resume`. It forwards `--json` / `--rpc` and, for an interactive resume, `--no-fullscreen`, `--no-mouse`, and `--hover`.
...
Attach or resume the newest dated session. Use this when you want recency instead of `continue`'s active-or-single-running resolution. Detached sessions are dated by lockfile start time; lockfile-less interactive sessions are dated by the newer mtime of `state.json` and `summary.json`.
...
|---|---|---|---|
| `--project <dir>` | path | cwd | Project directory. |
| Workflow flags | — | — | Same options as `continue`, including `--json`, `--rpc`, `--auto`, `--allow-hooks`, `--allow-repo-runners`, `--no-fullscreen`, `--no-mouse`, `--hover`, runner overrides, mode, budget, and approval controls. Stream flags apply only to interrupted resumable state; TUI flags apply to both live attach and interactive interrupted-resume paths. |
...
- **Reads:** `.splitbrief/sessions/` (to find the most recent), plus each session's `lockfile.json` and the mtimes of `state.json` / `summary.json` when no lockfile exists.
- **Writes:** same as `attach` or `resume` depending on session state.
...
- Selects the greatest session sort key regardless of status: `lockfile.json.startTimeMs` when a lockfile exists, otherwise the newer mtime of `state.json` and `summary.json`. Empty session directories with none of those files are skipped.
- If the most recent session is running, attaches. If interrupted, resumes. Interactive paths in both branches honor `--no-fullscreen`, `--no-mouse`, and `--hover`; interrupted resumes also honor `--json` / `--rpc`.
...
- [docs/CHANGELOG.md](/docs/reference/changelog) — schema versions.
...
splitbrief handoff agents-md --task T001,T003,T007 --out ./pack
...
- [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) — when to hand off vs. continue in SPLITBRIEF.
- [TASK-CONTRACT.md](/docs/reference/task-brief-format) — contract every renderer must respect.
...
- [DEBUGGING.md](/docs/guides/debugging-a-run) — reverting bad runs.
...
- [SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) — `/approval` semantics during a run.
- [HOOKS-CONFIG.md](/docs/guides/hooks) — hook policies that interact with approvals.
...
- [ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) — MCP integration in the engine.
...
- `0` — always (read-only). An empty result prints `No SPLITBRIEF worktrees found.`
...
| `--delete-branch` | boolean | `false` | Also force-delete the `splitbrief/<name>` branch with `git branch -D` after removal. This destructive branch deletion does not require `--force`; that flag controls only the worktree guards. |
| `--project <dir>` | path | cwd | Project directory. |
...
# Remove a clean, idle worktree and force-delete its branch
splitbrief worktree remove migration --delete-branch

# Bypass the worktree guards and force-delete the branch
...
- **Writes:** `git worktree add/remove`; `--delete-branch` runs `git branch -D`.

### See also (worktree)

- `splitbrief start --worktree` — create one.
- [WORKTREES.md](/docs/guides/worktrees-and-parallel-sessions) — design rationale.
...
- `--delete-branch` always uses force deletion (`git branch -D`), whether or not worktree removal also uses `--force`. If the worktree directory is already absent but the branch remains, the command prunes stale worktree metadata and can still force-delete that branch.
...
| Flag | Type | Default | Description |
|---|---|---|---|
| `<session-id>` | string \| number (positional) | auto-resolved | Specific session ID or numeric alias from `splitbrief ps`. Optional when exactly one session is running. |
| `--project <dir>` | path | cwd | Project directory. |
| `--no-fullscreen` | boolean | fullscreen on for an interactive TTY | Keep the attached TUI on the normal screen. This also disables mouse tracking and hover. |
| `--no-mouse` | boolean | mouse on when fullscreen | Disable Ink mouse tracking while leaving fullscreen enabled. This also disables hover. |
| `--hover` | boolean | `false` | Opt in to hover highlighting; effective only with an interactive TTY, fullscreen, and mouse tracking. |
...
# Pick by the # column from splitbrief ps
splitbrief attach 1
...
- Numeric positional values resolve against the same recency-ordered aliases shown in the `splitbrief ps` `#` column.
...
- **Reads:** `.splitbrief/sessions/<id>/lockfile.json` for every session directory; for lockfile-less sessions, the mtimes of `state.json` and `summary.json`.
- **Writes:** none.
...
- `#` is a numeric alias (1, 2, 3...) usable with `splitbrief attach 1`, `splitbrief detach 1`, `splitbrief continue 1`, and commands whose `--session` option accepts an alias. A bare empty session directory has `-` instead because it cannot be resolved by alias.
...
- Not supported on Windows: prints `splitbrief attach/detach/ps are not supported on Windows.` and exits `1`.
- A session whose lockfile is missing reports `STATUS=unknown`, `PID=-`, `MODE=-`, `FEATURE=-`. If it has `state.json` or `summary.json`, it still receives an alias and participates in recency ordering; a completely empty session directory remains unaliased.
- Sorting uses `lockfile.json.startTimeMs` when available. Without a lockfile, it uses the newer mtime of `state.json` and `summary.json`; a session with none of those timestamps sorts at `0`.
...
| `--no-fullscreen` | `start`, `resume`, `continue`, `last`, `attach` | fullscreen on for an interactive TTY | Keep the TUI on the normal screen. This also disables mouse tracking and hover. No effect in JSON, RPC, or detached-server execution. |
| `--no-mouse` | `start`, `resume`, `continue`, `last`, `attach` | mouse on when fullscreen | Disable TUI mouse tracking and therefore hover. No effect when no TUI is rendered. |
| `--hover` | `start`, `resume`, `continue`, `last`, `attach` | `false` | Enable hover only when an interactive TUI also has fullscreen and mouse tracking. |
...
| Mode | Nominal pre-build planner calls | Approval gates | Best for |
|---|:---:|:---:|---|
...
| `speckit` | 5–6 | supporting spec + plan + briefs | Large, risky, or externally visible work. |

`full` is a legacy alias for `speckit`. A successful run adds one planner call for final review after implementation. Detailed semantics in [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
Schemas and YAML shape live in [ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) and [CONFIGURATION.md](/docs/reference/configuration). Built-in CLI tools may run their own auto/permission modes according to their upstream behavior; SPLITBRIEF surfaces warnings but does not sandbox shell or network access.
