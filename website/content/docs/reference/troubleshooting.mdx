...
description: Symptom-driven guide. Find the line that matches what you see, follow the diagnosis, apply the fix.
---
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [CONTRIBUTING.md](https://github.com/b4r7x/splitbrief/blob/main/CONTRIBUTING.md).
...
**See also:** [docs/PRINCIPLES.md](https://github.com/b4r7x/splitbrief/blob/main/docs/PRINCIPLES.md), [docs/STRUCTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/STRUCTURE.md).
...
**See also:** [docs/BOOTSTRAP.md](https://github.com/b4r7x/splitbrief/blob/main/docs/BOOTSTRAP.md), [docs/CONFIGURATION.md](/docs/reference/configuration).
...
**See also:** [CLI-REFERENCE.md](/docs/reference/cli#splitbrief-doctor), [CONFIGURATION.md](/docs/reference/configuration).
...
**See also:** [FEATURES.md](https://github.com/b4r7x/splitbrief/blob/main/docs/FEATURES.md#run-readiness--doctor), [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
**See also:** [FEATURES.md](https://github.com/b4r7x/splitbrief/blob/main/docs/FEATURES.md#splitbrief-start---worktree-name), [WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
**Likely cause:** The planner is configured with an expensive model (commonly Opus) and the implementer uses the same model. Standard and speckit make 4 and 5–6 nominal pre-build planner calls respectively; a successful run then adds one final-review call. Multiplying that by Opus pricing escalates fast.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md), [docs/VISION.md](/docs/project/why-splitbrief-comparison).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), `src/engine/orchestrator/budget/`.
...
**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md), [docs/OTEL.md](/docs/guides/observability-otel).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), `src/core/providers/known-models.ts`.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys), [docs/REPOMAP.md](/docs/guides/repo-map).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
**See also:** [docs/REPOMAP.md](/docs/guides/repo-map), [docs/CONFIGURATION.md](/docs/reference/configuration).
...
**See also:** [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format), [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
**See also:** [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format), [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), `src/engine/orchestrator/final-review.ts`.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), `src/engine/orchestrator/task/loop.ts`.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/DEBUGGING.md](/docs/guides/debugging-a-run).
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [docs/DEBUGGING.md](/docs/guides/debugging-a-run).
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys).
...
**See also:** [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys), [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §approval, [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §approval.tiers.
...
**See also:** [docs/WORKTREES.md](/docs/guides/worktrees-and-parallel-sessions), `src/engine/snapshots/` (lock implementation).
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/INVARIANTS.md](https://github.com/b4r7x/splitbrief/blob/main/docs/INVARIANTS.md).

---

## Drift

### Symptom: Drift report marks an intended file as out of scope

**Likely cause:** Accepted scope is not matched by substring. A changed path must exactly match a task's `file`, `scope.inBounds`, or `scope.approvedOutOfBounds` entry, or match SPLITBRIEF's limited glob forms: `dir/**`, `dir/*`, `*.ext`, or a single `*` between a prefix and suffix. By contrast, each `scope.outOfBounds` entry is checked as a literal substring against changed paths and the raw diff; it is not treated as a glob or regular expression.

**Fix:**
1. Inspect `.splitbrief/sessions/<id>/drift-report.json`: `out_of_scope_file` means no accepted-scope entry matched, while `out_of_bounds_text_match` names the literal exclusion that appeared in a changed path or diff.
2. Add an exact project-relative path to `scope.inBounds`, or use one of the supported glob forms when the task intentionally owns several files.
3. If the change is a reviewed exception, add its exact path to `scope.approvedOutOfBounds` and re-run the final review.
4. Remove glob syntax from `scope.outOfBounds`; write a literal path or text fragment there, and avoid short fragments that can also appear in unrelated diff content.

**Prevention:** Prefer exact project-relative paths (`src/engine/orchestrator/run/workflow.ts`). Use the limited glob forms only for intentional path families, and keep literal `scope.outOfBounds` entries specific.

**See also:** [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format), `src/engine/orchestrator/final-review.ts`.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), `src/engine/orchestrator/final-review.ts`.
...
**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md), `src/engine/orchestrator/`.
...
2. Confirm the source session still has a valid `.splitbrief/sessions/<id>/state.json`; handoff renders from that persisted task state, plus any available spec, plan, and constitution files.
3. Fix the reported renderer, permissions, or path error. If you wrote a custom renderer, also verify that it returns safe relative output paths.
4. Regenerate the default pack cleanly with `splitbrief handoff '<target>' --session '<id>' --mode overwrite`. For a partial custom output outside `.splitbrief/` that has no recognizable manifest, choose a fresh `--out` directory instead.

**Prevention:** Treat handoff packs as regenerable exports of persisted session state. Keep custom renderers deterministic, and use a fresh output directory or the documented write modes when exporting again.

**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format).
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), `src/engine/handoff/`.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration).
...
**See also:** [docs/API-KEYS.md](/docs/guides/api-key-security), [docs/CONFIGURATION.md](/docs/reference/configuration).
...
**Fix:**
1. If the branch contains work you still want, rename it first. If it is safe to delete, run `git branch -D 'splitbrief/<slug>'`.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), [docs/CONFIGURATION.md](/docs/reference/configuration).

---

### Symptom: `Failed to start server: timeout waiting for server to accept connections`

**Likely cause:** The parent did not observe both readiness signals before the startup deadline: a live `.splitbrief/sessions/<id>/lockfile.json` and a connectable Unix-domain socket at `.splitbrief/sessions/<id>/ipc.sock`. The deadline is 5 seconds for the built JavaScript server and 20 seconds when startup goes through `tsx`. A readable lockfile with `exitedAt` produces the separate `server exited during startup (...)` error. A child that exits before recording a valid lockfile, or a missing, invalid, dead, or stale lockfile, can instead fall through to this timeout without a socket connection attempt.

**Fix:**
1. Check `.splitbrief/sessions/<id>/server.log` for the crash reason.
2. Inspect `lockfile.json` and `ipc.sock` in that session directory. A lock without a connectable socket means IPC startup did not finish; neither file means the child did not reach lockfile creation before the deadline.
3. Run `splitbrief ps` to rule out another live session in the checkout, then retry `splitbrief start --detach "..."`.
4. If the error reports that the Unix-domain socket path exceeds its byte limit, use a shorter project directory or worktree name; that is a distinct startup failure, not a TCP port conflict.

**Prevention:** Avoid launching concurrent sessions in the same checkout and keep project/worktree paths short enough for Unix-domain sockets. Use the built CLI for routine detached runs; source-mode `tsx` startup deliberately receives the longer deadline.
...
**See also:** [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys), `src/core/runtime/commands/registry.ts`.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §Review gates.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/OTEL.md](/docs/guides/observability-otel).
...
**Likely cause:** The hook command exited non-zero. SPLITBRIEF handles the failure according to the hook's `on_failure` setting: `block` halts a pre-hook gate, `warn` continues with a warning, and `ignore` continues silently. Post-hooks report failures without blocking the workflow.
...
4. If the hook is advisory, set `on_failure: warn` or `on_failure: ignore`, or change it to exit `0`.

**Prevention:** Treat hooks as deterministic and idempotent; test them in isolation before wiring them into the workflow.

**See also:** [docs/HOOKS-CONFIG.md](/docs/guides/hooks), [docs/DEBUGGING.md](/docs/guides/debugging-a-run).
...
2. If the binary needs a custom `PATH`, put the environment setup and command in an executable wrapper script, then set the hook's `command` to that script path.
3. Do not add `env.PATH` to the hook entry. The current strict hook schema accepts `kind`, `name`, `command`, `args`, `timeout_ms`, and `on_failure` for command hooks, but no per-hook `env` field.

**Prevention:** Always use absolute paths in hook commands; never assume the interactive shell's `PATH`.

**See also:** [docs/HOOKS-CONFIG.md](/docs/guides/hooks).
...
**See also:** [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) ("CRITICAL — NEVER COMMIT, NEVER STAGE"), `.claude/hooks/block-git-commits.sh`.
...
**Fix:**
1. Use the task ID from the brief as a commit prefix: `git commit -m "T003: rename foo to bar"`.
...
2. Parse stdout line by line as public records. The first record is `{ type: "readiness_report", report: ... }`; subsequent engine events are wrapped as `{ type: "event", data: <EngineEvent> }`.
...
**See also:** [docs/DEBUGGING.md](/docs/guides/debugging-a-run), [docs/CONFIGURATION.md](/docs/reference/configuration).
...
2. To gate CI on drift, post-process `summary.json` and exit non-zero from your wrapper script when the cleanliness score falls below your threshold.
3. Use `driftSummary.score`, `driftSummary.errorCount`, or `driftSummary.warningCount` as the numeric signal. `driftSummary.score` is a cleanliness score: `1` is clean and lower is worse. `chainDriftSummary.score` is the opposite signal: higher means a stronger repeated cross-task drift chain.

**Prevention:** Wire drift checks into your CI script explicitly; do not assume SPLITBRIEF will fail the build for you.

**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases), `src/engine/orchestrator/final-review.ts`.

---

### Symptom: Want CI to fail when the drift cleanliness score falls below a threshold

**Likely cause:** No built-in flag enforces a minimum drift cleanliness score; you wire it yourself.
...
2. Read `driftSummary.score`, `driftSummary.errorCount`, or `driftSummary.warningCount`. A `driftSummary.score` of `1` is clean and lower is worse; `chainDriftSummary.score` runs in the opposite direction and rises with stronger repeated drift chains.
3. Exit non-zero from the wrapper if the cleanliness score falls below your threshold:
   `node -e "const s=JSON.parse(require('fs').readFileSync(process.argv[1])); process.exit((s.driftSummary?.score ?? 1) < 0.7 ? 1 : 0)" '.splitbrief/sessions/<id>/summary.json'`.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/MIGRATION.md](/docs/reference/migration).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/BOOTSTRAP.md](https://github.com/b4r7x/splitbrief/blob/main/docs/BOOTSTRAP.md).
