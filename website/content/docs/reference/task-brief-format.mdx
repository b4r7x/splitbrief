...
description: The stable Task Brief v1 contract, transport shape, evidence ledger, and deterministic drift report.
---
...
A branded string whose scalar schema enforces exactly `^T\d{3}$`. It accepts any three-digit
suffix and does not enforce starting at `T001`, sequential allocation, or a historical reuse
policy. Parsing a complete `tasks.md` file does separately reject duplicate IDs within that task
set.

The bundled format example uses `T001`, and producers may allocate zero-padded IDs in sequence.
That is a producer convention, not a guarantee imposed by `TaskIdSchema`. Consumers should treat
task IDs as opaque identifiers rather than deriving task order from the numeric suffix.
...
- **ID stability**: retries keep the current in-memory task ID. Task regeneration is
  planner-authored, and no runtime reconciliation layer matches title/file pairs to preserve or
  replace IDs. Runtime validation enforces the ID shape and task-set uniqueness, not identity
  continuity across regenerated Task Briefs.
...
External tools may attach their own metadata in `state.external` under a namespaced key. SPLITBRIEF does not read or validate these fields — they are opaque passthrough metadata. For example, this valid JSON value can be assigned to `state.external`:
...
```bash
# Pick a session
session_id=$(ls -t .splitbrief/sessions/ | head -1)
jq '.tasks[0]' ".splitbrief/sessions/${session_id}/state.json"
```
...
- [Workflow modes & phases](/docs/concepts/workflow-modes-and-phases) §1.1 — full phase-transition table.
- [Sessions & persistence](/docs/concepts/sessions-and-persistence) — session folder structure.
...
Workflow runs that execute Task Briefs write a per-session evidence ledger to
`.splitbrief/sessions/<id>/evidence.json` (constant `EVIDENCE_FILE`, file mode `0o600`).
The ledger is a workflow-run artifact, not an artifact emitted by every CLI command:
read-only commands and the standalone `spec` command do not create one. It is the durable
record of what each task was supposed to prove and what was actually observed, written
incrementally as tasks reach a terminal state and amended once final review completes.
...
type EvidenceLedger = {
  ...
  tasks: Array<{
    ...
    validation: Array<{
      ...
      passed: boolean;
      errorSummary?: string;
      retryState?: 'initial-failure' | 'retry' | 'escalated' | 'failed';
      changedFiles?: string[];
    }>;
  ...
  approvals?: Array<{
    ts: string;
    tier: 'confirm';
    actionClass: 'read' | 'write_in_scope' | 'validation' | 'write_out_of_scope' | 'destructive' | 'network' | 'package_change';
...
`method: 'mcp-tool'` records completion reported through SPLITBRIEF's constrained MCP evidence
tools. Per-validation `retryState` identifies the attempt or escalation phase, while its optional
`changedFiles` captures the files associated with that validation observation. Top-level
`approvals` and `rejections` preserve confirm approvals and sticky/confirm denials with their
action classification and reason.
...
|---|---|---|
| `out_of_scope_file` | info for a pre-existing or legacy-ledger-unattributed file; otherwise warning (or error if any task declares `outOfBounds`) | Changed file is not accepted by any task target, in-bounds pattern, or approved-out-of-bounds pattern. |
