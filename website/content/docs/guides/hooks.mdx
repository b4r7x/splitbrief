---
title: Hooks
description: Run commands or modules at workflow lifecycle boundaries, with explicit trust and failure policies.
---
...
`post_task` currently has no `file` field. This command therefore formats the project (`.`), not only files touched by the completed task.
...
| `post_task`      | After each implementer task succeeds        | `taskId`, `title`, `method`, `retries`, `duration`                   |
| `pre_validation` | Before tsc/lint/test runs                   | `taskId`, `status`, `passed`, `stages`                               |
...
| `post_commit`    | After optional product-level git commit succeeds | `taskId`, `message`, `file?`                                    |
| `pre_escalation` | Before planner escalation runs              | `taskId`                                                             |
| `pre_compact`    | Reserved for transcript compaction (FUTURE) | (none)                                                               |
| `on_error`       | Every emitted `error` engine event          | `message`                                                            |
...
`on_error` follows the event stream: it runs for every emitted `error` event, including errors the workflow can recover from. Its dispatch does not imply that the workflow has terminated.
...
**Timeout:** Module hooks use the same timeout defaults as command hooks: `timeout_ms` defaults to 30000 ms and is capped at 300000 ms. Module calls are raced against a `setTimeout`. The timer can abandon async work or a promise that never settles once the event loop gets a turn; it cannot preempt synchronous work. A module that enters an infinite loop or performs a long blocking operation stalls the process before the timer can fire. When the timer does fire, the promise is abandoned rather than killed, and the outcome follows `on_failure`.
...
|----------------------|----------------------------------------|------------------------------------------|
| `${event.file}`      | task_started, git_commit               | Project-relative path string             |
...
`task_started` backs `pre_task`; `git_commit` backs both `pre_commit` and `post_commit`. The `task_completed` event behind `post_task` has no `file` field.
...
Which placeholders resolve depends on the event type. Missing fields collapse to empty strings. Every dispatched event also exposes `${event.type}`, `${event.ts}`, and `${event.phase}`; the table lists the event-specific fields.
...
| `post_task`       | `${event.taskId}`, `${event.title}`, `${event.method}`, `${event.retries}`, `${event.duration}` |
| `pre_validation`  | `${event.taskId}`, `${event.status}`, `${event.passed}`, `${event.stages}`         |
...
```json
{
  ...
  "context": {
    ...
    "sessionId": "2026-04-14-add-jwt-middleware"
  }
}
```
...
- `context` contains `projectDir` (absolute) and `sessionId` (per-invocation). On `pre_commit`, it also contains `files`, the task's changed-file paths.
...
For awaited `pre_*` hooks, stderr is surfaced with the hook outcome. The current fire-and-forget `post_*` / `on_*` sink does not publish stderr as a separate warning, so use a structured stdout outcome when a post-hook diagnostic must reach the workflow.
...
Awaited `pre_*` hooks finish before their guarded action continues. `post_*` and `on_*` dispatch is fire-and-forget, so hooks from different events can overlap even though entries within one event stay sequential.
...
| Module's async work exceeds `timeout_ms`         | Promise abandoned once the timer fires. Treated as failure per `on_failure`.                 |
| Module blocks synchronously or loops forever     | The timer cannot preempt it; the process remains blocked because JS cannot terminate in-process code. |
...
`timeout_ms` is mandatory at the schema level (default 30000 ms, hard ceiling 300000 ms). It bounds command hooks and async module hooks, but it is not a preemption boundary for synchronous in-process module code.
...
2. **Mandatory timeout.** `timeout_ms` has a default (30000 ms) and a hard ceiling (300000 ms). Command hooks are terminated when they exceed it; module hooks are abandoned only if the event loop can run the timer, so synchronous module code remains inside the process trust boundary.
...
`prettier-on-change` is registered on `post_task`, but `task_completed` currently has no `file` field. The implementation returns `allow` without running Prettier when that field is absent, so this built-in currently no-ops pending file attribution.
...
On `pre_commit`, scans every attributed path from `ctx.files`, falling back to `event.file` when no attributed set is available. It uses the shared secret-redaction pattern set, which covers provider tokens, bearer/basic authorization values, private-key material, and common key/token assignments.
...
3. SPLITBRIEF runs the workflow. After each task succeeds, Prettier formats the project.

This working form formats the whole project because `post_task` has no `event.file`. No script is needed — the hook invokes `npx` directly.
