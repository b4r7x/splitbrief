---
title: Cookbook
description: Copy-paste recipes for SPLITBRIEF workflows, safety controls, handoffs, CI, hooks, and TUI operations.
---
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration), [docs/BOOTSTRAP.md](https://github.com/b4r7x/splitbrief/blob/main/docs/BOOTSTRAP.md), [docs/API-KEYS.md](/docs/guides/api-key-security).
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.3.1, recipe 3.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.2.
...
phase: reviewing-spec  ← awaiting approval
  approve | Ctrl+E inline edit | edit-file external edit | comment <text> revises | quit
> approve
...
`standard` is the default mode. It makes 4 nominal pre-build planner calls; a successful run adds one final-review call after implementation. The spec gate blocks by default (`approve: spec`). Type `approve` to continue, `comment <text>` to send feedback that triggers regeneration, or `quit` to reject. `Ctrl+E` opens the built-in inline editor. Type `edit-file` when you explicitly want the external editor; spec and plan review currently also register a legacy external-editor handler on the `Ctrl+E` chord. `VISUAL` is explicit; otherwise SPLITBRIEF uses non-terminal `EDITOR`, detected GUI editors from safe absolute `PATH` segments, macOS `open -W -t`, terminal `EDITOR`, and finally `vi`; Windows detection honors `PATHEXT` plus `.cmd`, `.exe`, and `.bat` shims.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.3, recipe 5.
...
Speckit makes 5–6 nominal pre-build planner calls and writes `research.md`, `spec.md`, `clarifications.md`, `constitution-check.json`, `plan.md`, `analyze.json`, and `tasks.md` to the session directory. A successful run adds one final-review planner call after implementation. Constitution violations with `severity: hard` abort the run.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.3.2, [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) `workflow.maxBudget`, recipe 8.
...
The combined run is `unpriced`: LM Studio is local, while CLI subscription usage has no per-token price available to SPLITBRIEF. `cost_update` events refresh cumulative token counters only; they do not contain dollar fields. Dollar totals appear only when pricing is known, so SPLITBRIEF does not turn an unpriced runner into a fabricated `$0`.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §planner / §implementer.
...
```bash
set -o pipefail
jq 'select(.type == "cost_update")' '.splitbrief/sessions/<session-id>/session.jsonl' | tail -20
...
By phase
  ...
  specifying           $0.22
  planning             $0.35
  implementing         $0.04 · local
  final-review         $0.11
By task
  add CSV exporter     7.3k tok
```
...
Cost History (3 sessions)
  ...
  By Provider:
    Anthropic:  $0.90 (3 sessions)
```

The live drilldown derives phase and task rows from cumulative token usage. A `cost_update` record has `type`, `ts`, `phase`, and a `tokenUsage` object; it does not carry dollar totals. Final dollar fields live in `summary.json.costBreakdown`, and unpriced rows stay labeled `local`, `unpriced`, or `price unknown` rather than receiving an invented amount.

```jsonl
{"type":"cost_update","ts":1745692902300,"phase":"implementing","tokenUsage":{"plannerInput":12000,"plannerOutput":1800,"implementerInput":6400,"implementerOutput":900,"escalationInput":0,"escalationOutput":0}}
```
...
**See also:** [docs/DEBUGGING.md](/docs/guides/debugging-a-run) §Event log, recipe 36.
...
`kind: cli` spawns `claude` as a subprocess with the brief on stdin. Spend goes through your subscription, so SPLITBRIEF labels its pricing state `unpriced`; `local` is reserved for local providers such as Ollama and LM Studio.

**Variations:** Other CLI tools: `tool: codex`, `opencode`, `aider`, `copilot`, `kilo-code`. Each is auto-detected by `splitbrief init`.

**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) §7 Runner abstraction.
...
**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) §6 Persistence layout, recipes 11–13.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §snapshots, [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §Auto-snapshots.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §approval, recipes 15, 25.
...
**Variations:** Inspect the final deterministic report directly: `cat '.splitbrief/sessions/<session-id>/drift-report.json' | jq`. Inspect `drift-chains.json` for cross-task chain state. The summary screen shows a `Drift` row with the latest score.

**See also:** [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format) §Drift detection rules.
...
phase: reviewing-briefs
  ...
  approve | Ctrl+E inline edit | e/edit/edit-file external edit | comment <text> revises | reject
> Ctrl+E
```

`Ctrl+E` opens the built-in field editor over the focused Task Brief. Save it with `Ctrl+S`; SPLITBRIEF re-runs brief quality and keeps the review gate open until you approve. Type `e`, `edit`, or `edit-file` when you want the persisted `tasks.md` in an external editor instead. `VISUAL` is explicit; otherwise SPLITBRIEF uses non-terminal `EDITOR`, detected GUI editors from safe absolute `PATH` segments, macOS `open -W -t`, terminal `EDITOR`, and finally `vi`; Windows detection honors `PATHEXT` plus `.cmd`, `.exe`, and `.bat` shims.
...
**See also:** [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) `/revise-spec`, recipe 17.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.1, [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys).
...
`Ctrl+E` opens the built-in field editor over the focused brief. Type `e`, `edit`, or `edit-file` to open `.splitbrief/sessions/<id>/tasks.md` in the external editor. `VISUAL` is explicit; otherwise SPLITBRIEF uses non-terminal `EDITOR`, detected GUI editors from safe absolute `PATH` segments, macOS `open -W -t`, terminal `EDITOR`, and finally `vi`; Windows detection honors `PATHEXT` plus `.cmd`, `.exe`, and `.bat` shims. After either edit path saves, SPLITBRIEF re-runs brief quality and returns to the brief-review gate until you explicitly approve.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) `workflow.briefReview`.
...
The cleanest way is a `pre_task` module hook that returns `kind: deny`. Add to `.splitbrief/config.yaml`:
...
**See also:** [docs/HOOKS-CONFIG.md](/docs/guides/hooks) §`kind: module`, recipe 39.
...
**See also:** [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) `/redo-task`, [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.1.
...
**Variations:** `splitbrief resume` re-enters the active saved phase. Use `splitbrief continue <session-id>` when the active pointer is absent but you know the session id. `planning`, `implementing`, and `final-review` are intrinsically resumable. An earlier non-terminal phase such as `researching` or `specifying` is also resumable when it carries `awaitingContinue: true` from an intentional interruption or a durable `pendingRecovery`; without either override, resume refuses that phase with a clear error.

**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.5, §1.6.
...
Handoff written to: .splitbrief/handoffs/claude-code
  ...
  plan.md
  CLAUDE.md
  .claude/agents/splitbrief-handoff.md
```

The Claude Code renderer emits the shared handoff files, a root `CLAUDE.md`, and
`.claude/agents/splitbrief-handoff.md`. The writer also creates `manifest.json` in the output
directory; the CLI's per-file list reports renderer files and does not repeat that generated
manifest.
...
**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) §1 Handoff packs, recipes 24–27.
...
The `copilot-issue` renderer writes one Markdown issue body. It embeds selected task IDs, titles, files, acceptance criteria, constraints, and validation commands in `issue.md`; it does not emit `tasks/<id>.md` files. The writer adds `manifest.json` beside it.
...
```
Handoff written to: .splitbrief/handoffs/spec-kit
  README.md
  tasks/T001.md
  tasks/T002.md
  spec.md
...
`spec-kit` is the default target (`splitbrief handoff` with no argument). The writer adds
`manifest.json`; its top-level `briefHash` lets downstream tools detect drift.
...
**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) §8 `engine/handoff/`.
...
The pack contains only the requested task, but the supporting spec / plan are still included so the receiving agent has context. The writer adds `manifest.json` beside the renderer files.

**Variations:** `--task T001,T003,T005` for a subset. Without `--task`, every task is included.

**See also:** [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) `/handoff`.
...
**See also:** [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) §1 MCP server.
...
**Variations:** `--worktree` (no value) auto-slugifies the feature description. Worktrees do not isolate dev-server ports or deliberately shared/global dependency caches. Worktree-local untracked `node_modules` directories remain separate — see [Worktrees & parallel sessions](/docs/guides/worktrees-and-parallel-sessions) for mitigations.
...
**See also:** [docs/WORKTREES.md](/docs/guides/worktrees-and-parallel-sessions).
...
**See also:** recipe 33, [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) §1 IPC server.
...
**See also:** recipe 34, [docs/DEBUGGING.md](/docs/guides/debugging-a-run).
...
**Variations:** `cat '.splitbrief/sessions/<session-id>/server.log'` for the full server log; `cat '.splitbrief/sessions/<session-id>/session.jsonl' | jq '.type'` for the event stream up to the crash.

**See also:** [docs/DEBUGGING.md](/docs/guides/debugging-a-run), recipe 36.
...
**You'll see (abridged NDJSON; every displayed line is a complete record):**

```jsonl
{"type":"readiness_report","report":{"status":"ready"}}
{"type":"event","data":{"type":"workflow_started","ts":1745692800000,"phase":"researching","feature":"regenerate API client from openapi.yaml"}}
{"type":"event","data":{"type":"planner_status","ts":1745692801200,"phase":"researching","status":"running"}}
{"type":"event","data":{"type":"brief_quality_passed","ts":1745692840100,"phase":"planning","score":1,"warningCount":0}}
{"type":"event","data":{"type":"task_completed","ts":1745692902300,"phase":"implementing","taskId":"T001","title":"Regenerate API client","method":"local","retries":0,"duration":42100}}
{"type":"event","data":{"type":"workflow_complete","ts":1745692980000,"phase":"complete"}}
```

`--json` requires a feature argument and replaces the TUI sink with NDJSON-on-stdout. The first record is a readiness report; subsequent workflow events use `{ "type": "event", "data": <EngineEvent> }` envelopes. The on-disk JSONL log is still written. Headless JSON runs auto-approve workflow review gates and answer clarification questions with an empty string. File-write tiered approvals remain governed by `approval` config and fail closed when a sticky or confirm decision would need input. `--allow-hooks` authorizes the configured hooks without showing the interactive trust prompt.
...
**You'll see responses on stdout (abridged NDJSON):**
...
{"type":"status","data":{"pending":"approval","approvalType":"spec","filePath":"/repo/.splitbrief/sessions/2026-04-26-add-auth-audit-logging/spec.md"}}
...
```bash
set -o pipefail
splitbrief start --json "validate the generated client" 2>/dev/null > events.ndjson

# Cumulative token usage from the final cost update
jq 'select(.type == "event" and .data.type == "cost_update") | .data.tokenUsage' \
  events.ndjson | tail -1

# Final dollar breakdown
jq '.costBreakdown | {planner: .actualPlannerCost, implementer: .actualImplementerCost, total: .totalActualCost}' \
  '.splitbrief/sessions/<session-id>/summary.json'

# Per-task cost
jq '.taskBreakdown[] | {task: .taskId, method: .method, cost: .cost}' \
  '.splitbrief/sessions/<session-id>/summary.json'
```
...
```jsonl
{"plannerInput":12000,"plannerOutput":1800,"implementerInput":6400,"implementerOutput":900,"escalationInput":0,"escalationOutput":0}
{"planner":0.41,"implementer":0,"total":0.41}
{"task":"T001","method":"local","cost":0}
{"task":"T002","method":"escalated-full","cost":0.04}
```

`cost_update` events carry cumulative tokens in `.data.tokenUsage`; they do not carry dollar totals. Final dollar amounts live in `summary.json.costBreakdown`, and per-task costs live in `summary.json.taskBreakdown`.

**Variations:** Filter for workflow failures: `jq 'select(.type == "event" and (.data.type == "task_full_fail" or .data.type == "error"))' events.ndjson`.

**See also:** [docs/DEBUGGING.md](/docs/guides/debugging-a-run) §Event log, recipe 8.
...
```bash
set -o pipefail
splitbrief start --json --allow-hooks --approve none "validate the generated client" | tee events.ndjson
status=$?

if jq -e 'select(.type == "event" and .data.type == "budget_exceeded")' events.ndjson > /dev/null; then
...
**You'll see (abridged NDJSON):**

```jsonl
{"type":"event","data":{"type":"budget_warning","ts":1745692940000,"phase":"implementing","currentCost":1.20,"maxBudget":1.50}}
{"type":"event","data":{"type":"budget_exceeded","ts":1745692960000,"phase":"implementing","currentCost":1.51,"maxBudget":1.50}}
```

The script then prints the CI diagnostic:
...
**Variations:** `--json` already auto-approves spec, plan, and Task Brief review gates, so `--approve none` is not required to prevent those prompts. Use quick mode when you want the smaller one-call planning flow, not as a workaround for headless brief approval. Tiered file-write approvals can still fail closed and recovery requirements still exit non-zero.
...
**See also:** [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) §1.3.2.

---

### 40. Run a post-task shell hook

**When:** you want to run Biome after every completed task without modifying the validation pipeline.
...
```bash
splitbrief start "add the User model"
```
...
The current `task_completed` event exposes `taskId`, `title`, `method`, `retries`, and `duration`, but no `file`. Do not use `${event.file}` in `post_task`: it expands to an empty string. For the same reason, the current `prettier-on-change` built-in returns without formatting on this event. The example uses the project root explicitly.

A post hook observes an action that has already completed, so `on_failure: block` would only be informational and cannot undo or abort the task. Keep `warn`, or move a check that must block into a suitable `pre_*` hook such as `pre_validation`.

**Variations:** Pass `--allow-hooks` in non-TTY CI to authorize hooks without a prompt. Editing the hooks block or a module hook file invalidates trust. Use `kind: module` for in-process JS / TS hooks. On a `pre_*` event, `on_failure: block` aborts the upcoming action; `warn` (default) logs; `ignore` is silent. Mandatory `timeout_ms` ceiling: 300_000 ms.

**See also:** [docs/HOOKS-CONFIG.md](/docs/guides/hooks), recipe 19.
...
```bash
OTEL_TRACES_EXPORTER=console splitbrief start --mode quick "smoke test"
```
...
function shutdownProvider() {
  shutdownPromise ??= provider.forceFlush().finally(() => provider.shutdown());
  return shutdownPromise;
}

process.once('beforeExit', () => void shutdownProvider());
for (const signal of ['SIGINT', 'SIGTERM']) {
  process.once(signal, () => void shutdownProvider());
}
```

Node's `--import` flag ensures the bootstrap runs before the main script, so the TracerProvider is registered before SPLITBRIEF starts. The explicit `forceFlush()` then `shutdown()` path drains a custom batch processor before process exit; without it, the final batch can be lost.
...
node --import ./otel-bootstrap.mjs ./node_modules/.bin/splitbrief start "trace the checkout workflow"
```
...
**See also:** [docs/OTEL.md](/docs/guides/observability-otel), [docs/DEBUGGING.md](/docs/guides/debugging-a-run) §OpenTelemetry.
...
**See also:** [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys).
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §approval.tiers, recipe 14.
...
**See also:** [docs/CONFIGURATION.md](/docs/reference/configuration) §approval, [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) `/revise-plan`.

---

## See also

- [docs/WORKFLOW.md](/docs/concepts/workflow-modes-and-phases) — phase machine, modes, abort/queue/continue.
- [docs/CONFIGURATION.md](/docs/reference/configuration) — every config field with type, default, description.
- [docs/SLASH-COMMANDS-REFERENCE.md](/docs/reference/slash-commands-and-keys) — full reference for the runtime slash commands.
- [docs/HOOKS-CONFIG.md](/docs/guides/hooks) — hook events, payloads, security model.
- [docs/ARCHITECTURE.md](https://github.com/b4r7x/splitbrief/blob/main/docs/ARCHITECTURE.md) — canonical inventory of CLI commands, runners, sinks, paths.
- [docs/DEBUGGING.md](/docs/guides/debugging-a-run) — JSONL inspection, headless mode, OTel console exporter.
- [docs/WORKTREES.md](/docs/guides/worktrees-and-parallel-sessions) — what worktrees do and don't isolate.
- [docs/OTEL.md](/docs/guides/observability-otel) — span hierarchy, exporter setup, dual-resolution caveat.
- [docs/TASK-CONTRACT.md](/docs/reference/task-brief-format) — Task Brief schema, drift-report rules, evidence vocabulary.
