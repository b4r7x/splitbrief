...
export function SummaryScreen({ summary, theme, onDone, onSlashCommand, onOpenOverlay, errorMessage, onClearError }: SummaryScreenProps) {
  useInput((input, key) => {
    if (input === '?') {
      onOpenOverlay?.('help');
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box flexDirection="column" marginTop={1}>
        <Box><Box width={20}><Text color={theme.textDim}>Feature</Text></Box><Text bold>{summary.feature}</Text></Box>
        <Box><Box width={20}><Text color={theme.textDim}>Time</Text></Box><Text>{formatTime(summary.totalTime)}</Text></Box>
        {summary.plannerName && <Box><Box width={20}><Text color={theme.textDim}>Planner</Text></Box><Text>{summary.plannerName}</Text></Box>}
        {summary.implementerName && <Box><Box width={20}><Text color={theme.textDim}>Implementer</Text></Box><Text>{summary.implementerName}</Text></Box>}
        <Box><Box width={20}><Text color={theme.textDim}>Savings</Text></Box><Text bold color={theme.success}>{summary.estimatedCostSavings}</Text></Box>
      </Box>

      <Box flexDirection="column" marginTop={1}>
        <Box>
          <Text color={theme.success}>{progressBar(completed, summary.totalTasks, 30)}</Text>
      ...
      {summary.costBreakdown && (
        <Box flexDirection="column" marginTop={1}>
          <Box><Box width={20}><Text color={theme.textDim}>Actual cost</Text></Box><Text bold>{formatCost(summary.costBreakdown.totalActualCost)}</Text></Box>
          <Box><Box width={20}><Text color={theme.textDim}>Saved</Text></Box><Text color={theme.success}>{formatCost(summary.costBreakdown.savingsAmount)} ({summary.costBreakdown.savingsPercentage.toFixed(0)}%)</Text></Box>
          <Box><Box width={20}><Text color={theme.textDim}>Local rate</Text></Box><Text color={theme.success}>{(summary.costBreakdown.localCompletionRate * 100).toFixed(0)}%</Text></Box>
        </Box>
      )}
