...
// A test client that authenticates, then answers every recovery_needed prompt with the
// supplied action — the same round-trip a detached TUI client performs over the socket.
...
// One done task + one failed task, as the orchestrator would persist after a failing run.
...
describe('runWorkflowLoop detached retry', () => {
  it('retry re-runs only failed work from the freshly saved state, not boot-time undefined', async () => {
    ...
    const fakeRunWorkflow = async (opts: RunWorkflowOptions): Promise<Summary> => {
      ...
      if (call === 1) {
        // Boot run completes with a failure and persists the failed state to disk.
    ...
    // The boot run starts from no saved state.
    expect(observedSavedStates[0]).toBeUndefined();

    // The retry must re-enter with the freshly saved state (fails today: undefined),
    // with the failed task reset to pending and the done task preserved — re-running
    // only failed work instead of re-planning from scratch.
    ...
    // The persisted retry state on disk matches what the retry run re-entered with.
