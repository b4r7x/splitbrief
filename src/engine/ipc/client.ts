...
import type { IpcPromptRequest, IpcPromptResponse, ServerMessage } from './protocol.js';
...
export type IpcClientStatus = 'connecting' | 'connected' | 'reconnecting' | 'failed' | 'detached';
...
export function handleServerMessage(msg: ServerMessage, callbacks: ServerMessageCallbacks): void {
  ...
  if (msg.kind === 'session_meta') {
    ...
    if (msg.payload.type === 'replay_complete') callbacks.resetAttempts();
    onEvent(msg.payload);
