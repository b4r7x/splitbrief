...
export function stateForRetryProfile(
  state: WorkflowState,
...
export async function createRetryRuntime(
  ctx: EscalationContext,
  profileOverride: string | undefined,
): Promise<RetryRuntime> {
  ...
  const profile = resolveImplementerProfiles(ctx.config).profiles.find(
    (candidate) => candidate.name === profileOverride,
  );
