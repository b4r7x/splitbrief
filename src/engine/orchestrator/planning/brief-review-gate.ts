...
import { formatBriefReadinessBlocks } from './brief-readiness-gate.js';
...
export function publishReadinessBlockWarning(opts: {
  ...
  publishWarning({
    ...
    safety: { category: 'workflow', code: 'brief_readiness_block' },
  });
}

export function recordReadinessOverride(opts: { bus: EventBus; phase: Phase }): void {
  publishWarning({
    bus: opts.bus,
    phase: opts.phase,
    message: 'Task Brief readiness override recorded; proceeding with the blocked briefs.',
    safety: { category: 'workflow', code: 'brief_readiness_overridden' },
  });
}
