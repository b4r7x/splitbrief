...
export function recomputeValidationSummary(tasks: EvidenceTask[]): EvidenceLedger['validationSummary'] {
...
export function withUpdatedTask(
  ...
  return { ...ledger, tasks, validationSummary: recomputeValidationSummary(tasks), generatedAt: nowIso() };
}
...
export function findOrSeed(ledger: EvidenceLedger, task: Task, briefHash?: string | null): EvidenceTask {
