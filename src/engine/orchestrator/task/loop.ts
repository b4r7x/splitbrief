...
export async function runTaskLoop(opts: RunTaskLoopOptions): Promise<TaskLoopResult> {
  ...
  for (let i = state.currentTaskIndex; i < totalTasks; i++) {
    ...
    if (hasDependencyFailed(task, failedTaskIds, skippedTaskIds)) {
      ...
      const review = await reviewTaskIfNeeded({ wctx, state, setTrackedState, task, taskIndex: i, filesTouched: issue.files, taskBreakdowns });
      state = review.state;
      return { state, taskBreakdowns, status: 'stopped' };
    }
    const conflictAction = await checkUserEditConflicts({ projectDir, sessionId, callbacks, bus: wctx.bus, state, task, taskIndex: i, baseline: changedFilesBaseline, acknowledgedUserEditFiles, setTrackedState });
    state = conflictAction.state;
    if (conflictAction.stopped) {
      const review = await reviewTaskIfNeeded({ wctx, state, setTrackedState, task, taskIndex: i, filesTouched: state.pendingRecovery?.files ?? [], taskBreakdowns });
      state = review.state;
      return { state, taskBreakdowns, status: 'stopped' };
    }
    ...
    const routingDecision = routeTaskToImplementerProfile({ task: refreshedTask, context: wctx.context, profiles: routingProfiles, languageContext: buildProjectLanguageContext(projectDir, state.discoveredValidation?.language) });
    ...
    if (!selectedProfile) {
      ...
      publishRecoveryPrompted(wctx.bus, issue);
      setTrackedState(state);
      const review = await reviewTaskIfNeeded({ wctx, state, setTrackedState, task: refreshedTask, taskIndex: i, filesTouched: issue.files, taskBreakdowns, routingDecision });
      state = review.state;
      return { state, taskBreakdowns, status: 'stopped' };
    }
    const selectedTaskConfig = taskConfigForProfile(config, selectedProfile);

    await maybeAutoSnapshot({ projectDir, sessionId, config, bus: wctx.bus, phase: state.phase, enabled: config.snapshots?.auto?.preTask === true, taskIndex: i, label: `pre-task-${i}` });

    const taskImplementer = await createTaskImplementer({ wctx, profile: selectedProfile, taskConfig: selectedTaskConfig, singleImplementerMode });
    const { implementerModel: _previousImplementerModel, ...stateWithoutImplementerModel } = state;
    const taskState: WorkflowState = { ...stateWithoutImplementerModel, implementerTool: getRunnerDisplayName(selectedProfile.config), ...(selectedModel !== undefined && { implementerModel: selectedModel }) };
    const taskWorkflowContext: WorkflowContext = { ...wctx, config: selectedTaskConfig, implementer: taskImplementer, implementerProfile: selectedProfile.name, routingDecision };

    const taskAcceptedFiles = new Set<string>();
    state = await runSingleTask({ wctx: taskWorkflowContext, task: refreshedTask, taskCodeRefreshed: true, index: i, totalTasks, state: taskState, taskBreakdowns, setTrackedState, setCurrentTask, onTaskAcceptedFiles: (files) => { for (const file of files) taskAcceptedFiles.add(file); } });
    const completedTask = state.tasks[i];
    if ((completedTask?.status === 'done' || completedTask?.status === 'escalated') && taskAcceptedFiles.size === 0) {
      for (const file of await inferTaskAcceptedChangedFiles(projectDir, refreshedTask)) taskAcceptedFiles.add(file);
    }
    const absorbedFiles = new Set([...taskAcceptedFiles, ...acknowledgedUserEditFiles]);
    changedFilesBaseline = await refreshChangedFilesBaseline({ projectDir, baseline: changedFilesBaseline, absorbedFiles });
    acknowledgedUserEditFiles.clear();

    const reviewDecision = await reviewTaskIfNeeded({ wctx: taskWorkflowContext, state, setTrackedState, task: completedTask ?? refreshedTask, taskIndex: i, filesTouched: [...taskAcceptedFiles], taskBreakdowns, routingDecision, implementerProfile: selectedProfile.name });
    ...
    const postTaskConflictAction = await checkUserEditConflicts({ projectDir, sessionId, callbacks, bus: wctx.bus, state, task: refreshedTask, taskIndex: i, baseline: changedFilesBaseline, acknowledgedUserEditFiles, setTrackedState });
    state = postTaskConflictAction.state;
    if (postTaskConflictAction.stopped) {
      const review = await reviewTaskIfNeeded({ wctx: taskWorkflowContext, state, setTrackedState, task: refreshedTask, taskIndex: i, filesTouched: state.pendingRecovery?.files ?? [], taskBreakdowns, routingDecision, implementerProfile: selectedProfile.name });
      state = review.state;
      setCurrentTask(undefined);
      return { state, taskBreakdowns, status: 'stopped' };
    }
    if (acknowledgedUserEditFiles.size > 0) {
      changedFilesBaseline = await refreshChangedFilesBaseline({ projectDir, baseline: changedFilesBaseline, absorbedFiles: acknowledgedUserEditFiles });
      acknowledgedUserEditFiles.clear();
    }

    const succeeded = completedTask?.status === 'done';
    await maybeAutoSnapshot({ projectDir, sessionId, config, bus: wctx.bus, phase: state.phase, enabled: succeeded && config.snapshots?.auto?.postTask === true, taskIndex: i, label: `post-task-${i}`, recordInRunLedger: true });

    const budgetCheck = await checkBudgetAfterTask({ wctx, state, taskBreakdowns, totalTasks, budgetWarningEmitted, budgetPauseEmitted });
