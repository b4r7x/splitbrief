...
export function readDriftReport(projectDir: string, sessionId: string): DriftReport | null {
  const target = driftReportPath(projectDir, sessionId);
  if (!existsSync(target)) return null;
  try {
    const parsed = JSON.parse(readFileSync(target, 'utf8')) as DriftReport;
    return { ...parsed, briefHash: parsed.briefHash ?? null };
  } catch {
    return null;
  }
}
