...
import type { ExplainArtifactInputs, ReadinessSummary, RunExplainArtifact } from './explain-types.js';
...
async function readJson(projectDir: string, sessionId: string, file: string): Promise<unknown | null> {
  try {
    return JSON.parse(await readFile(join(sessionDir(projectDir, sessionId), file), 'utf8')) as unknown;
  } catch (err) {
