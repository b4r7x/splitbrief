...
export async function captureChangedFilesBaseline(projectDir: string, files?: string[]): Promise<ChangedFilesBaseline> {
...
export async function changedFilesSinceBaseline(projectDir: string, baseline: ChangedFilesBaseline): Promise<string[]> {
...
export async function inferTaskAcceptedChangedFiles(projectDir: string, task: Task): Promise<string[]> {
  const patterns = [
    task.file,
    ...(task.scope?.inBounds ?? []),
