import type { RoutingDecision, RouteTaskOptions } from './types.js';
import { assessProfile } from './assessment.js';
import { compareProfileRouteRank, rejectionReason, selectedReason, costPosture } from './helpers.js';
...
function noCapableProfileReason(hasCredentialFailure: boolean, hasCapabilityFailure: boolean): string {
  if (hasCredentialFailure) return 'No credential-usable implementer profile satisfies this task capability and context requirements';
  if (hasCapabilityFailure) return 'No capable implementer profile satisfies this task capability and context requirements';
