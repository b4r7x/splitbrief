...
function classifyFile(opts: {
  ...
  if (affectedDependencies.length > 0) {
    return {
      ...
      affectedTaskIds: uniqueTaskIds(affectedDependencies),
    };
  }
  ...
  if (affectedFutureTasks.length > 0) {
    return {
      ...
      affectedTaskIds: uniqueTaskIds(affectedFutureTasks),
    };
  }
...
export function classifyUserEditConflict(opts: {
  ...
  const affectedTaskIds = uniqueTaskIds(fileConflicts.flatMap((conflict) => conflict.affectedTaskIds));
