...
describe('extractCode', () => {
  it('extracts code from ```typescript fenced block with high confidence', () => {
    ...
    expect((result as any).code).toContain('const x = 1;');
    expect((result as any).code).toContain('export function foo()');
  });
  ...
  it('extracts code from bare ``` fenced block with high confidence', () => {
    ...
    expect('code' in result).toBeTruthy();
    expect(result.confidence).toBe('high');
    expect((result as any).code).toContain('import { something }');
  });
  ...
  it('uses the longest block when multiple code blocks exist', () => {
    ...
    expect('code' in result).toBeTruthy();
    expect(result.confidence).toBe('high');
    expect((result as any).code).toContain('readFile');
    expect((result as any).code).toContain('writeFile');
  });
  ...
  it('extracts raw code starting with import as high confidence', () => {
    ...
    expect('code' in result).toBeTruthy();
    expect(result.confidence).toBe('high');
    expect((result as any).code).toBe(response);
  });
  ...
  it('strips surrounding explanation text and extracts code with medium confidence', () => {
    ...
    expect((result as any).code).toContain('import { foo }');
    expect((result as any).code).toContain('export const bar');
  });
