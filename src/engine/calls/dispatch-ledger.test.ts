...
describe('task dispatch ledger', () => {
  it('starts fresh and records the claim on the snapshot', () => {
    ...
    expect(ledger.claimDispatch(createTaskCompilationAttemptId())).toMatchObject({
      kind: 'claimed',
    });
  ...
  it('allows claims 1 through 64 and refuses claim 65', () => {
    ...
    expect(ledger.claimDispatch(createTaskCompilationAttemptId())).toEqual({
      kind: 'refused',
  ...
  it('reconstructs from explicit durable state without changing the frozen envelope', () => {
    ...
    expect(reconstructed.claimDispatch(createTaskCompilationAttemptId())).toMatchObject({
      kind: 'refused',
      reason: 'dispatch-limit',
      dispatchCount: 64,
    });
