...
import { buildPaletteResults } from './palette-aggregate.js';
import type { PaletteInputs } from './palette-aggregate.js';
...
describe('buildPaletteResults', () => {
  describe('empty query', () => {
    it('returns all items in source-priority order without MRU', () => {
      const inputs = makeInputs({
        slashItems: [{ label: 'Run', description: 'Run it', shortcut: null, action: noop, availableOn: [] }],
  ...
  describe('MRU ordering', () => {
    ...
    it('within MRU, rank 1 (most recently used) comes first', () => {
      const inputs = makeInputs({
        ...
        mruIds: ['slash:Runner', 'slash:run'],
      });
      const results = buildPaletteResults(inputs);
      const mruResults = results.filter((r) => r.mruRank > 0);
      expect(mruResults[0]?.id).toBe('slash:Runner');
      expect(mruResults[1]?.id).toBe('slash:run');
    });
    ...
    it('mruRank is 1 for index 0 in mruIds, 0 for absent ids', () => {
      const inputs = makeInputs({
        ...
        mruIds: ['slash:Run', 'slash:Other'],
      });
      const results = buildPaletteResults(inputs);
      const run = results.find((r) => r.id === 'slash:Run');
      const deploy = results.find((r) => r.id === 'slash:Deploy');
  ...
  describe('id format', () => {
    it('produces correct id prefixes for each source', () => {
      const inputs = makeInputs({
        slashItems: [{ label: 'Run', description: '', shortcut: null, action: noop, availableOn: [] }],
      ...
      expect(ids).toContain('slash:Run');
  ...
  describe('does not mutate inputs', () => {
    it('does not mutate input arrays', () => {
      ...
      const originalOrder = slashItems.map((i) => i.label);
      buildPaletteResults(makeInputs({ query: 'run', slashItems }));
      expect(slashItems.map((i) => i.label)).toEqual(originalOrder);
    });
  });

  describe('shortcut field', () => {
    it('carries shortcut from slash items', () => {
      const inputs = makeInputs({
        slashItems: [{ label: 'Run', description: 'run it', shortcut: 'ctrl+r', action: noop, availableOn: [] }],
      });
      const results = buildPaletteResults(inputs);
    ...
    it('non-slash sources have null shortcut', () => {
      const inputs = makeInputs({
