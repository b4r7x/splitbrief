  ...
  try {
    ...
    if (outcome.kind === 'crash') {
      bus.publish({
        ...
        ts: Date.now(),
        phase,
        message: `${label} crashed: ${outcome.message}`,
      });
      ...
      bus.publish({
        ...
        ts: Date.now(),
        phase,
        message: `${label} denied (post-hook deny is informational only): ${outcome.message ?? ''}`,
      });
    }
  } catch (err) {
    bus.publish({
      ...
      ts: Date.now(),
      phase,
      message: `${label} crashed: ${toErrorMessage(err)}`,
    });
  }
}
...
function getEventPhase(event: EngineEvent): Phase {
  return eventPhase(event) ?? 'implementing';
}
