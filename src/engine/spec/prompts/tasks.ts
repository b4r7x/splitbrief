...
7. **Constraints** — \`### Constraints\`: invariants, dependency rules, ${ctx.importConvention}, refusal conditions.
...
5. **Inline type definitions**: Copy referenced ${ctx.typeAnnotationStyle} or data shape definitions verbatim into \`### Type Definitions\`. Max ~300 tokens — prioritize definitions that appear in the function signature.
...
export function buildTasksPrompt(spec: string, plan: string, languageContext?: LanguageContext): string {
