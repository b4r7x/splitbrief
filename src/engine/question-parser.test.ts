...
describe('extractQuestionsFromStream', () => {
  it('extracts a single choice question', () => {
    ...
    expect(questions[0].text).toBe('Which auth?');
    expect(questions[0].options).toEqual(['JWT', 'Sessions']);
    expect(questions[0].default).toBe(0);
  });
  ...
  it('extracts a single input question', () => {
    ...
    expect(questions[0].type).toBe('input');
    expect(questions[0].text).toBe('What is the table name?');
    expect(questions[0].default).toBe('users');
  });
  ...
  it('extracts a single confirm question', () => {
    ...
    expect(questions[0].text).toBe('Enable SSR?');
