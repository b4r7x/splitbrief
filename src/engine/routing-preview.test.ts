...
describe('buildWorkerPacketPreview', () => {
  ...
  it('resolves a cache-only model context length when a contextCache is provided', () => {
    const task = makeTask();
    const config: Config = {
      ...makeConfig(),
      implementerProfiles: {
        default: 'cache-worker',
        profiles: {
          'cache-worker': {
            kind: 'api',
            provider: 'openrouter',
    ...
    const withoutCache = buildWorkerPacketPreview({ task, context: defaultContext, config });
    expect(withoutCache?.contextLength).toBe(8192);

    const withCache = buildWorkerPacketPreview({
      task,
    ...
    expect(withCache?.contextLength).toBe(64_000);
  });
