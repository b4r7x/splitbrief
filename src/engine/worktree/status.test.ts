...
describe('listWorktrees', () => {
  ...
  it('returns one entry with name, branch, status, and sessionId after createWorktree', async () => {
    ...
    expect(entry.branch).toBe('splitbrief/feat-d');
    expect(entry.status).toBe('none');
    expect(entry.sessionId).toBeNull();
  });

  it('reports status "active" when .splitbrief/active exists and state is implementing', async () => {
  ...
  it('reports status "active" for a non-terminal session with no .splitbrief/active pointer', async () => {
