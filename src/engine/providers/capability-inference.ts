...
const ANTHROPIC_IMAGE_MODEL_RE = /claude-(?:(opus|sonnet|haiku)-[3-9]|[3-9](?:[-.]\d+)?-(opus|sonnet|haiku))/i;
...
export function modelSupportsEffort(provider: ProviderId, model: string | undefined): boolean {
  ...
  if (provider === 'openai' || provider === 'openrouter') return /^(o[1345]|gpt-[5-9])/i.test(key) || /(?:^|-)r1(?:-|$)|reasoner/i.test(key);
