...
describe("streamCompletion", () => {
  ...
  it("forwards reasoning_effort when effort is set", async () => {
    let captured: { reasoning_effort?: string | undefined } | null = null;
    const client: MockClient = {
      chat: {
        completions: {
          create: async (body) => {
    ...
    await streamCompletion(client, "o3-mini", [{ role: "user", content: "hi" }], {
      temperature: 0.2,
      onProgress: () => {},
      effort: "high",
    });
    expect((captured as { reasoning_effort?: string } | null)?.reasoning_effort).toBe("high");
  });

  it("omits reasoning_effort when effort is undefined", async () => {
    let captured: { reasoning_effort?: string | undefined } | null = null;
    const client: MockClient = {
      chat: {
        completions: {
    ...
    expect((captured as { reasoning_effort?: string } | null)?.reasoning_effort).toBeUndefined();
  });
});
