...
function truncateByLines(
  ...
  return {
    text: [
      ...
      `${DISPLAY_TRUNCATION_MARKER} ${countNoun(omitted, 'line')} hidden`,
    ].join('\n'),
    truncated: true,
  };
}
...
function truncateByChars(
  ...
  const marker = `\n${DISPLAY_TRUNCATION_MARKER} ${countNoun(text.length - maxChars, 'char')} hidden`;
...
function previewProjectContext(projectDir: string): ProjectContext {
