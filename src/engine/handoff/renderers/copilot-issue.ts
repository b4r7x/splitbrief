...
export function renderCopilotIssue(input: HandoffInput): HandoffPack {
  const taskList = input.tasks
    .map(t => `- **${t.id}** — ${t.title} (\`${t.file}\`, \`${t.action}\`) → [tasks/${t.id}.md](tasks/${t.id}.md)`)
    .join('\n');
  ...
  const constraints = input.tasks
    .flatMap(t => t.constraints.map(c => `- ${t.id}: ${c}`))
    .join('\n') || '- none declared';

  const validationLines: string[] = [];
  if (input.validation?.typecheck) validationLines.push(`- typecheck: \`${input.validation.typecheck}\``);
