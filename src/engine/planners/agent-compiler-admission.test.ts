...
const OPERATION_ID = TaskCompilationOperationIdSchema.parse('operation-agent-admission');
...
function invocationFixture(): PreparedPlannerInvocation {
  return {
    runtime: {
      executablePath: '/usr/bin/fake-agent-compiler',
...
function agentScript(logPath: string): string {
  return [
    "const fs = require('fs');",
...
describe('legacy agent planner compiler admission', () => {
  it('constructs the legacy agent planner without ambient session fallback', () => {
  ...
  it('refuses compiler batch dispatch with a typed capability failure and zero batch spawns', async () => {
    const projectDir = createTempDir('agent-admission-compiler');
    ...
    const invoke = createLegacyCommandInvoke({
      ...
      args: ['-e', agentScript(logPath)],
      outputFormat: 'text',
      notFoundMessage: 'Agent planner command not found',
      backendId: 'agent',
    });
    try {
      const failure = await runMultiPhasePlanning(
        {
          invokePlan: invoke,
          backendKind: 'cli',
          runnerName: 'agent-admission-test',
          compiler: { invocation: invocationFixture(), ledger },
        },
        {
          feature: 'agent admission feature',
          projectDir,
          callbacks: {
            onOutput: () => {},
            onPhase: () => {},
  ...
  it('refuses a declared-file compiler transport before spawning the agent command', async () => {
    const projectDir = createTempDir('agent-admission-declared-file');
    ...
    const invoke = createLegacyCommandInvoke({
      ...
      notFoundMessage: 'missing agent command',
      backendId: 'agent',
    });
    try {
      ...
      expect(String(result.error?.message)).toContain('legacy agent planner');
  ...
  it('refuses a detached-fresh compiler scope before spawning the agent command', async () => {
    const projectDir = createTempDir('agent-admission-detached');
    const sentinel = join(projectDir, 'child-spawned');
      ...
      const result = await invoke({
        ...
        callContext: {
          ...
          sessionScope: {
            kind: 'detached-fresh',
            operationId: OPERATION_ID,
            programId: TaskCompilationProgramIdSchema.parse('program-agent-admission'),
            batchId: TaskCompilationBatchIdSchema.parse('batch-agent-admission'),
            attemptId,
          },
        },
        callbacks: { onOutput: () => {} },
  ...
  it('refuses a compiler call envelope before spawning the agent command', async () => {
    const projectDir = createTempDir('agent-admission-envelope');
    const sentinel = join(projectDir, 'child-spawned');
    const callContext = createPlannerCallContext({}, 'planner');
    const invoke = createLegacyCommandInvoke({
      command: process.execPath,
  ...
  it('carries the claimed batch attempt identity on refusal', async () => {
    ...
    const callContext = {
      ...
      sessionScope: {
        ...
        operationId: OPERATION_ID,
        programId: TaskCompilationProgramIdSchema.parse('program-agent-identity'),
        batchId: TaskCompilationBatchIdSchema.parse('batch-agent-identity'),
        attemptId,
      },
    };
    const invoke = createLegacyCommandInvoke({
