...
describe('createOtelSink', () => {
  ...
  it('sets workflow_config attributes on the workflow span', () => {
    ...
    sink({ type: 'workflow_config', ts: 2, phase: 'idle', mode: 'standard', plannerTool: 'claude-code', plannerModel: 'opus', implementerTool: 'ollama', implementerModel: 'llama3' });
    sink({ type: 'workflow_complete', ts: 100, phase: 'complete' });
  ...
  it('emits a task span nested under phase on task_started → task_completed', () => {
    ...
    sink({ type: 'task_started', ts: 10, phase: 'implementing', taskId: 'T1' as never, title: 'Add foo', index: 0, total: 1, file: 'a.ts', action: 'create' });
    sink({ type: 'task_completed', ts: 50, phase: 'implementing', taskId: 'T1' as never, title: 'Add foo', method: 'local', retries: 0, duration: 40 });
    sink({ type: 'workflow_complete', ts: 100, phase: 'complete' });
  ...
  it('marks task span with skip_reason on task_skipped', () => {
    const sink = createOtelSink({ provider });
    sink({ type: 'workflow_started', ts: 1, phase: 'idle', feature: 'x' });
    sink({ type: 'task_started', ts: 10, phase: 'implementing', taskId: 'T3' as never, title: 'Old task', index: 0, total: 1, file: 'c.ts', action: 'create' });
    sink({ type: 'task_skipped', ts: 30, phase: 'implementing', taskId: 'T3' as never, title: 'Old task', reason: 'already done' });
    sink({ type: 'workflow_complete', ts: 100, phase: 'complete' });
  ...
  it('records workflow_cancelled with ERROR status on all open spans', () => {
    ...
    sink({ type: 'workflow_started', ts: 1, phase: 'idle', feature: 'x' });
    sink({ type: 'planner_status', ts: 2, phase: 'implementing', status: 'running' });
    sink({ type: 'task_started', ts: 10, phase: 'implementing', taskId: 'T4' as never, title: 'A', index: 0, total: 1, file: 'd.ts', action: 'create' });
  ...
  it('accumulates cost_update tokens on the workflow span', () => {
    const sink = createOtelSink({ provider });
    sink({ type: 'workflow_started', ts: 1, phase: 'idle', feature: 'x' });
    sink({ type: 'cost_update', ts: 20, phase: 'implementing', tokenUsage: { plannerInput: 100, plannerOutput: 50, implementerInput: 200, implementerOutput: 80, escalationInput: 10, escalationOutput: 5 } });
    sink({ type: 'workflow_complete', ts: 100, phase: 'complete' });
  ...
  it('is a no-op before workflow_started', () => {
    const sink = createOtelSink({ provider });
    sink({ type: 'cost_update', ts: 1, phase: 'idle', tokenUsage: { plannerInput: 1, plannerOutput: 1, implementerInput: 1, implementerOutput: 1, escalationInput: 0, escalationOutput: 0 } });
