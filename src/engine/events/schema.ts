...
function phaseEvent<T extends EngineEvent['type']>(type: T) {
  return z.object({
    type: z.literal(type),
    ts: z.number(),
    phase: PhaseSchema,
...
function noPhaseEvent<T extends EngineEvent['type']>(type: T) {
  return z.object({
    type: z.literal(type),
    ts: z.number(),
  });
...
const userEditConflictActionSchema = z.enum([
  'continue-unrelated',
  'regenerate-rebase',
  'pause',
  'skip-current-task',
...
const userEditConflictSchema = z.object({
  kind: z.enum([
    'unrelated',
...
const taskContextFitSchema = z.enum(['fits', 'tight', 'overflow']);
const currentCodeContextModeSchema = z.enum(['none', 'whole-file', 'function-level', 'truncated']);

const taskReviewRequestFields = {
  taskId: TaskIdSchema,
  taskTitle: z.string(),
...
const EngineEventDiscriminatedSchema = z.discriminatedUnion('type', [
...
export const EngineEventSchema = z.custom<EngineEvent>((value) =>
  EngineEventDiscriminatedSchema.safeParse(value).success
);
