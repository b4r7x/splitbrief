...
const KEY_PATTERNS: RedactionRule[] = [
  ...
  {
    pattern: /\b((?:[A-Z0-9_.-]*(?:API[_-]?KEY|TOKEN|PASSWORD|PASSWD|SECRET|CREDENTIAL|X-API-KEY|ACCESS[_-]?TOKEN|REFRESH[_-]?TOKEN)[A-Z0-9_.-]*)\s*[:=]\s*)(['"]?)[A-Za-z0-9._~+/=-]+\2(?=\s|$|[;,.)])/gi,
    replacement: marker => (_match, prefix: string, quote: string) => `${prefix}${quote}${marker}${quote}`,
  },
  {
    pattern: /(?<=(?:api[_-]?key|token|secret|credential|password|authorization|x-api-key)\s*[:=]\s*["']?)[a-zA-Z0-9+/=_-]{40,}/gi,
    replacement: marker => marker,
  },
  {
    pattern: /((?:api[_-]?key|token|secret|credential|password|authorization)\s*[=:]\s*["']?)[a-zA-Z0-9+/=_-]{20,}/gi,
    replacement: marker => (_match, prefix: string) => `${prefix}${marker}`,
  },
];

export function redactSecretsWithMetadata(msg: string, opts: RedactSecretsOptions = {}): RedactSecretsResult {
