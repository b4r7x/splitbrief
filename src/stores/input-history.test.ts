...
describe('inputHistoryStore', () => {
  ...
  it('ignores empty values', () => {
    inputHistoryStore.push('home', '   ');

    expect(inputHistoryStore.getEntries('home')).toEqual([]);
  });

  it('stores trimmed values with the newest first', () => {
    inputHistoryStore.push('home', 'first');
    inputHistoryStore.push('home', ' second ');

    expect(inputHistoryStore.getEntries('home')).toEqual(['second', 'first']);
  });
  ...
  it('deduplicates entries by moving repeats to the front', () => {
    ...
    expect(inputHistoryStore.getEntries('home')).toEqual(['first', 'second']);
  });
  ...
  it('skips consecutive duplicates', () => {
    ...
    expect(inputHistoryStore.getEntries('home')).toEqual(['first']);
  });

  it('caps history length', () => {
    for (let index = 0; index < inputHistoryStore.MAX_INPUT_HISTORY + 2; index++) {
      inputHistoryStore.push('home', `item-${index}`);
    }

    expect(inputHistoryStore.getEntries('home')).toHaveLength(inputHistoryStore.MAX_INPUT_HISTORY);
    expect(inputHistoryStore.getEntries('home')[0]).toBe('item-11');
  });
  ...
  describe('persistence', () => {
    ...
    it('load() caps entries at MAX_INPUT_HISTORY', async () => {
      const { readFileSync } = await import('node:fs');
      const lines = Array.from({ length: inputHistoryStore.MAX_INPUT_HISTORY + 3 }, (_, i) => `item-${i}`);
    ...
    it('push() triggers a debounced save after 300ms', async () => {
      ...
      inputHistoryStore.push('home', 'hello');
    ...
    it('push() debounces: multiple pushes within 300ms produce a single save', async () => {
      ...
      inputHistoryStore.push('home', 'second');
      vi.advanceTimersByTime(100);
      inputHistoryStore.push('home', 'third');
      vi.advanceTimersByTime(300);
