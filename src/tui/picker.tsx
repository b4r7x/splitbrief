...
import type { PlannerDetection, ImplementerDetection } from '../orchestrator/planner-detection.js';
...
function Picker({ planners, implementers, onComplete, onError }: PickerProps) {
  ...
  if (availablePlanners.length === 0) {
    ...
    return <Text color="red">No planner backends available.</Text>;
  }
  ...
  if (step === 'implementer' && allModels.length === 1 && selectedPlanner) {
    ...
    return (
      <Box flexDirection="column">
        <Text color="green">Auto-selected planner: {selectedPlanner}</Text>
        <Text color="green">Auto-selected model: {allModels[0].provider} / {allModels[0].model}</Text>
      </Box>
    );
  }
  ...
  if (step === 'planner') {
    ...
    return (
      <Box flexDirection="column">
        <Text bold>Select a planner backend:</Text>
        <Select
          options={options}
          onChange={(value) => {
  ...
  if (step === 'implementer' && selectedPlanner) {
    if (allModels.length === 0) {
      ...
      return (
        <Box flexDirection="column">
          <Text color="green">Planner: {selectedPlanner}</Text>
          <Text color="yellow">No local models detected. Configure implementer manually.</Text>
        </Box>
      );
    }
    ...
    return (
      <Box flexDirection="column">
        <Text color="green">Planner: {selectedPlanner}</Text>
        <Text bold>Select an implementer model:</Text>
        <Select
          options={options}
          onChange={(value) => {
  ...
  if (step === 'done' && selectedPlanner) {
    ...
    return <Text color="green">Planner: {selectedPlanner} (no local models — using defaults)</Text>;
  }
