...
const ACTION_ALIASES: Record<RecoveryAction, string[]> = {
  ...
  'planner-split-rebase': ['p', 'planner', 'split', 'rebase', 'split rebase', 'planner-split-rebase'],
...
export function formatRecoveryActionText(
  ...
  switch (action) {
    ...
    case 'route-bigger-worker': {
      const profile = factString(context.facts, 'routeBiggerProfile');
      return profile ? `route to bigger worker: ${profile}` : 'route to bigger worker';
    }
    case 'planner-split-rebase': {
      const verb = context.reason === 'user-edit-conflict' || context.reason === 'approval-promotion-conflict'
...
export function parseRecoveryActionAnswer(input: string, issue: RecoveryIssue): RecoveryAction {
  ...
  const action = input.length > 0 && input.trim().length === 0
    ? 'pause-run'
    : findAliasedAction(normalized);
...
function formatRecoveryHeader(issue: RecoveryIssue): string {
  const attemptSuffix = issue.attempts !== undefined
    ? ` after ${issue.attempts} attempt${issue.attempts === 1 ? '' : 's'}`
    : '';
...
function formatSubjectLines(issue: RecoveryIssue): string[] {
  ...
  if (issue.files.length > 0) lines.push(`Files: ${formatList(issue.files)}`);
  const affected = issue.affectedTaskIds.filter(taskId => taskId !== issue.taskId);
  if (affected.length > 0) lines.push(`Affected tasks: ${formatList(affected)}`);
  return lines;
}
...
function formatWorkerLines(issue: RecoveryIssue): string[] {
  ...
  if (!worker) return [];
  const contextLength = factNumber(issue.facts, 'contextLength');
  return [
...
function formatList(values: string[], maxItems = 3): string {
  const visible = values.slice(0, maxItems).join(', ');
  const hidden = values.length - maxItems;
  return hidden > 0 ? `${visible}, +${hidden} more` : visible;
}

function factString(facts: Record<string, RecoveryFact> | undefined, key: string): string | undefined {
...
function factNumber(facts: Record<string, RecoveryFact> | undefined, key: string): number | undefined {
  const value = facts?.[key];
