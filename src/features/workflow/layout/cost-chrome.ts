...
export function buildCostStatusLineLayout(input: CostStatusLineLayoutInput): CostStatusLineLayout {
  ...
  const cacheText = formatCacheHitPct(
    input.totalCacheRead > 0 ? input.totalCacheRead : undefined,
    input.totalInput,
  );
  ...
  const right = fitStatusParts(
    efficiencyParts,
    Math.max(0, contentWidth - left.length - STATUS_SPLIT_GAP),
  );
