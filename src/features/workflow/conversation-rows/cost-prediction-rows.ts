...
import { formatCostPredictionUnknownReasons, hasDisplayableCostPrediction, plannerEstimateReviewLine } from '../../../core/layout/cost-chrome.js';
...
export function costPredictionRows(
  ...
  if (!deterministic) {
    rows.push(row(`${keyPrefix}-heuristic`, 'Low: all local · Expected: ~15% escalation · High: ~40% escalation', 'textDim'));
    rows.push(row(
      `${keyPrefix}-costs`,
    ...
    if (reviewLine) rows.push(row(`${keyPrefix}-review`, reviewLine, prediction.plannerEstimateReview?.status === 'unavailable' ? 'warning' : 'planner'));
    return wrapRows(rows, width);
  }

  const unknownReasons = formatCostPredictionUnknownReasons(deterministic.totals.unknownCostReason);
  rows.push(row(`${keyPrefix}-desc`, 'Deterministic estimate from task briefs and routing preview', 'textDim'));
  rows.push(row(
    `${keyPrefix}-totals`,
