...
import { findLatestRenderableDiffEventIndex } from '../../../core/layout/event-sections.js';
import {
  ...
  useInput(
    (input, key) => {
      if (key.escape) {
        ...
        if (action.type !== 'none') { applyAction(action); return; }
      }

      const sections = getSections();

      const chord = handleWorkflowCtrlChords(input, key, isSmall, sections, findLatestRenderableDiffEventIndex);
      if (chord.type !== 'none') { applyAction(chord); return; }
