...
function formatLatestCheckpoint(summary: CheckpointSummaryRollup, maxLength: number, isSmall: boolean): string {
...
export function SummaryCheckpoints({ checkpointSummary }: SummaryCheckpointsProps) {
  ...
  const restoreCommand = commandFor(checkpointSummary.restoreCommand, 'restore', checkpointSummary.latestId);
  const summaryText = checkpointSummaryText(checkpointSummary, latest, runStatus, isSmall);
  const runStatusColor = runStatus === 'rejected'
    ? theme.warning
    : runStatus === 'accepted'
      ? theme.success
      : theme.textDim;
  ...
  return (
    <Box flexDirection="column" marginTop={isSmall ? 0 : 1}>
      <Text color={theme.textDim}>
        <Text bold color={theme.text}>Checkpoints:</Text>{' '}
        {summaryText}
      </Text>
      ...
      {isSmall && checkpointSummary.latestRunCheckpointId && checkpointSummary.latestRunCheckpointId !== checkpointSummary.latestId && (
      ...
      <Text color={runStatus === 'rejected' ? runStatusColor : theme.textDim}>
        restore: hash-guarded; conflicts skipped{isSmall ? '' : ' by default'}; --force destructive overwrite
      </Text>
    </Box>
  );
}
