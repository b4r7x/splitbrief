...
export const CALL_TUI_MAX_PUBLIC_PAYLOAD_BYTES = 512 * 1024;
...
export const CALL_TUI_MAX_PUBLIC_STRING_BYTES = 64 * 1024;
...
const CALL_CONSUMER_UNSUPPORTED_VALUE_PLACEHOLDER = '[... unsupported payload value omitted ...]';
const CALL_CONSUMER_CIRCULAR_REFERENCE_PLACEHOLDER = '[... circular payload reference omitted ...]';

export type CallConsumerContext = 'tui' | 'stdout-json' | 'stdout-text' | 'hooks' | 'session-log';
...
interface ConsumerPayloadPolicyOverrides {
  readonly maxBytes?: number | undefined;
  readonly maxStringBytes?: number | undefined;
}

interface ConsumerPayloadResult {
...
interface RedactedPublicPayloadResult {
  readonly payload: PublicPayload;
  readonly redacted: boolean;
}
...
const CONSUMER_POLICIES: Record<CallConsumerContext, ConsumerPayloadPolicy> = {
  ...
  // The `--plain` projection shares the stdout byte policy; only the label differs.
  'stdout-text': {
    context: 'stdout-text',
    maxBytes: CALL_STDOUT_JSON_MAX_PUBLIC_PAYLOAD_BYTES,
    maxStringBytes: CALL_STDOUT_JSON_MAX_PUBLIC_STRING_BYTES,
...
function boundPublicPayload(opts: {
  readonly payload: unknown;
...
export function boundConsumerPayload(opts: {
  readonly context: CallConsumerContext;
  readonly payload: unknown;
...
function measurePublicPayloadBytes(payload: PublicPayload): number {
