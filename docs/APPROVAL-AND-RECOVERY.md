...
**Repair before rejection.** A quality error is the planner's defect, not the reviewer's, so a failure at briefs approval spends one automatic repair round before the gate is final: `runBriefsApprovalLoop()` sends the blocking messages back to the planner as regeneration feedback, rewrites `tasks.md` from the reply, and re-gates it. If the regenerated briefs still fail on the next approval, the loop rejects immediately with a `brief_quality_rejected` error naming every blocking issue — it does not re-prompt up to the no-progress cap, because the gate is deterministic over unchanged briefs and no further approval could change the verdict. This is what keeps non-interactive transports (headless, RPC, MCP), whose approval callback always answers "approved", from spinning on a verdict they cannot influence. Editing `tasks.md` instead of approving keeps the human in control: an edit that fails the gate is reported and re-prompted, never overwritten by a regeneration.
