...
- **Engine-domain logic under `features/`.** A React-free module that re-runs engine work (route + prompt assembly + token estimate) sitting in `features/` because its only consumer is a preview panel. It is engine work — move the compute into `engine/` (or an `engine/facades/` entry) and let the feature call one function.
...
- [ ] `npm run test-ci` (typecheck → lint → test → invariants) is green.
