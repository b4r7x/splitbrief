...
**`src/app.tsx`** is the root component. It reads `routerStore` for the current screen (`home`, `workflow`, `summary`, `setup`) and `overlayStore` for any active overlay (help, command palette, settings, sessions, etc.). It builds a slash command context, binds keyboard handlers via `useAppKeys()`, and renders `<ThemeProvider>` wrapping `<Layout screen={...} overlay={...}/>`.
