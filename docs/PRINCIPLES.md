...
| 20 | **Hook system** — user-extensible workflow lifecycle commands (unrelated to React hooks) fire at workflow events (`pre_task`, `post_commit`, etc.); `post_planning` runs after Task Brief transport generation; trust prompt prevents RCE; 2 opt-in built-ins (`prettier-on-change`, `block-secrets`). | [HOOKS-CONFIG.md](./HOOKS-CONFIG.md) |
