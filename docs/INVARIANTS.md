...
| 31 | `rg -i "\b(cursor\|antigravity\|mimo-token-plan\|mistral\|gemini\|cerebras\|zai\|minimax\|moonshot\|dashscope\|llama-cpp)\b" src/features/runners src/app/overlays/runners.tsx --glob '*.tsx' --glob '!**/*.test.tsx'` | 0 | No vendor-ID JSX branches in runner picker surfaces — the `runners` feature plus its overlay page, any casing. Admission verdicts drive registry wiring, not UI conditionals; word boundaries keep TUI `isCursor` terminology out of the match. |
