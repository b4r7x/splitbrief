...
| 17c | `rg -n -P "[)\]}A-Za-z0-9_>'] as [A-Z][A-Za-z0-9_]*(?=\s*[;),<\].}>&\|]\|$\|\[)" src/ -g '*.ts' -g '*.tsx' -g '!**/*.test.ts' -g '!**/*.test.tsx' \| rg -v "\bas const\b\|\bas unknown\b\| as Extract<" \| rg -v "^src/(stores/use-stores\|engine/hooks/substitute\|utils/error)\.ts:"` | 0 matches | No incidental broad `as`-casts in production source (`as const` / `as unknown` / `as Extract` excluded; sanctioned files allow-listed) — [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
