...
| 8 | `rg "\bclass\s+\w+" src/ --glob '!**/*.test.ts' --glob '!**/*.test.tsx' \| rg -v "\bextends Error\b"` | 0 matches | Zero runtime classes in production source — errors via factory bags; `extends Error` subclasses sanctioned (D2, e.g. `CliError` in `src/cli/errors.ts`) — [ERRORS.md](./ERRORS.md) |
| 9 | `tsx scripts/import-boundaries.ts src` | 0 | Resolver-based import-boundary check (`scripts/import-boundaries.ts` `classify()`): no `src/features/<a>/** → src/features/<b>/**` cross-feature imports, no `src/components/** → src/features/**`, no `src/engine/** → src/{hooks,components,cli}/**`, no upward layer-rank import (target top-dir rank > source rank), and no value `src/stores/** → src/engine/**` import (`import type` is the only sanctioned cross channel). Depth-independent — [STRUCTURE.md](./STRUCTURE.md), [LAYERS.md](./LAYERS.md), [STORES.md](./STORES.md) |
...
| 20 | `rg -n "from '\.\.?/.*testing/helpers" src/ -g '*.test.ts' -g '*.test.tsx'` | 0 matches | Test files import `testing/helpers/**` via the `#testing/*` alias, never relative `../` paths — alias resolves through `package.json` `imports` + `tsconfig.test.json` `paths` — [TESTING.md](./TESTING.md) |
