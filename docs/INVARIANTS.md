...
| 4 | `rg "new Error\(" src/engine/ src/lib/ src/cli/ src/core/ \| rg -v "\.test\." \| rg -v "recordException\(new Error"` | 0 matches | Errors go through `error()` factory + domain bag; raw `new Error(` is banned in engine/lib/cli/core — [ERRORS.md](./ERRORS.md) |
...
| 17c | `rg -n -P "[)\]}A-Za-z0-9_>'] as [A-Z][A-Za-z0-9_]*(?=\s*[;),<\].}>&\|]\|$\|\[)" src/ -g '*.ts' -g '*.tsx' -g '!**/*.test.ts' -g '!**/*.test.tsx' \| rg -v "\bas const\b\|\bas unknown\b\| as Extract<" \| rg -v -P "^[^:]+:\d+:\s*(?://\|\*\|/\*)" \| rg -v "^src/(stores/use-stores\|engine/hooks/substitute\|utils/error\|utils/type-guards)\.ts:"` | 0 matches | No incidental broad `as`-casts in production source (`as const` / `as unknown` / `as Extract` excluded; comment lines excluded — a `// … as CLI` prose line is not a cast; sanctioned files allow-listed) — [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
...
| 31 | `rg -i "\b(cursor\|antigravity\|mimo-token-plan\|mistral\|gemini\|cerebras\|zai\|minimax\|moonshot\|dashscope\|llama-cpp)\b" src/features/runners src/app/overlays/runners.tsx src/features/crew src/features/settings src/app/overlays/settings.tsx src/app/screens/setup.tsx --glob '*.tsx' --glob '!**/*.test.tsx' -n \| rg -v -P '^[^:]+:\d+:\s*(?://\|\*\|/\*)'` | 0 | No vendor-ID JSX branches in seat-selection surfaces — the `runners`, `crew` and `settings` features plus the runners and settings overlay pages and the first-run setup screen, any casing (Crew's seat rows render inside Settings since T-058, so the standalone Crew overlay page is gone and the settings surfaces took its place). Admission verdicts drive registry wiring, not UI conditionals; word boundaries keep TUI `isCursor` terminology out of the match, and comment lines are excluded so prose such as `// cursor lead` is not a branch. |
...
Gates are consolidated here; full rationale for each lives in the linked doc. The runner registers 41 gates, with ids up to `'47'`.
...
- The engine must not import UI, React/Ink, stores, or Markdown components. If engine code needs Markdown logic, use only pure utilities from `src/utils/markdown/*`.
