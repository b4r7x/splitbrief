...
| 7 | `rg "process\.env(?:\.[A-Z_]+|\[['\"][A-Z_]+['\"]])\\s*=" src/engine/runners/agent-sdk --glob '!**/*.test.ts'` | 0 matches | Anthropic SDK key scoped via `env:` option, no global mutation — [LAYERS.md](./LAYERS.md) §SOTA provider decisions |
...
Gates are consolidated here; full rationale for each lives in the linked doc. The runner registers ids up to `'47'`; rows 36–45 (the T-065 controller-boundary scans) are not documented in the table above and are not backfilled here — read them from `scripts/check-invariants.ts` and `T065_RULES` until someone writes them up.
