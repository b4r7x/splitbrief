...
| 3 | `rg "useMemo\|useCallback\|React\.memo\|forwardRef\|useImperativeHandle" src/` | 0 matches | Zero memoization, no imperative handles — [STORES.md](./STORES.md), [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
...
| 6 | `rg "from 'simple-git'" src/ --glob '!**/*.test.ts' \| rg -v "^src/lib/git/client.ts:"` | 0 matches | Production `simple-git` imports stay in `src/lib/git/client.ts`; tests may import it directly for real git boundary coverage — [LAYERS.md](./LAYERS.md), [TESTING.md](./TESTING.md) |
| 8 | `rg "\bclass\s+\w+" src/ --glob '!**/*.test.ts' --glob '!**/*.test.tsx'` | 0 matches | Zero runtime classes in production source — [ERRORS.md](./ERRORS.md), [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
...
| 12b | `rg -ln "from 'react'\|from 'ink'" src/engine/ --glob '!**/*.test.ts' --glob '!**/*.test.tsx'` | 0 matches | Engine MUST NOT import React or Ink packages — [LAYERS.md](./LAYERS.md), [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
...
| 16 | `rg -n "from '\.\.?/" src/ --glob '*.ts' --glob '*.tsx' --glob '!**/*.test.ts' --glob '!**/*.test.tsx' \| rg -v "\.js'\|\.json'"` | 0 matches | Relative imports in production source must use `.js` extension — ESM resolution — [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
| 17 | `rg -n -P "[A-Za-z0-9_)\]]![^=)]" src/ -g '*.ts' -g '*.tsx' -g '!**/*.test.ts' -g '!**/*.test.tsx' \| rg -v "!==\|!=" \| rg -v "^src/(stores/use-stores\|engine/codebase/graph\|engine/codebase/pagerank)\.ts:"` | 0 matches | No incidental non-null (`!`) assertions in production source. Sanctioned files (`stores/use-stores`, `engine/codebase/graph`, `engine/codebase/pagerank`) are allow-listed — [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
| 17b | `rg -n -P ":\s*any\b(?![ ]?[a-z])\|<any>\|\bany\[\]\|\bas any\b\|Array<any>" src/ -g '*.ts' -g '*.tsx' -g '!**/*.test.ts' -g '!**/*.test.tsx'` | 0 matches | No explicit `any` in type positions in production source (prose excluded via terminator lookahead) — [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
| 17c | `rg -n -P "[)\]}A-Za-z0-9_>'] as [A-Z][A-Za-z0-9_]*(?=\s*[;),<\].}>&\|]\|$\|\[)" src/ -g '*.ts' -g '*.tsx' -g '!**/*.test.ts' -g '!**/*.test.tsx' \| rg -v "\bas const\b\|\bas unknown\b\| as Extract<" \| rg -v "^src/(stores/use-stores\|engine/hooks/substitute\|utils/error\|utils/type-guards)\.ts:"` | 0 matches | No incidental broad `as`-casts in production source (`as const` / `as unknown` / `as Extract` excluded; sanctioned files allow-listed) — [CLAUDE.md](https://github.com/b4r7x/splitbrief/blob/main/CLAUDE.md) |
| 18 | `npx knip --no-progress --no-config-hints --tags=-lintignore --include exports,types,files --reporter compact` | 0 matches | No dead files or unused exports/types across `src/`, `scripts/`, `evals/`, and `testing/` (fixtures negated, runtime-loaded). Knip reads `knip.json` (entry points, project glob, `ignoreDependencies`); it is not an inline ripgrep — [knip.json](https://github.com/b4r7x/splitbrief/blob/main/knip.json). `ignoreDependencies` carries `node-pty` because `testing/integration/cli/pty/capability.ts` imports it through a string constant so the PTY suite can skip itself where the native build is missing — knip's static analysis cannot follow that specifier, so the entry is load-bearing rather than stale. The reason lives here because `knip.json` is formatted by Biome, whose JSON parser rejects comments. |
| 19 | `npx depcruise --config .dependency-cruiser.cjs --output-type err-long src` | 0 errors | No runtime circular dependencies and full LAYERS.md layer-direction graph (utils→…→app/cli, one-way). Type-only cycles are erased at compile time and excluded via `viaOnly.dependencyTypesNot`. Backed by [.dependency-cruiser.cjs](https://github.com/b4r7x/splitbrief/blob/main/.dependency-cruiser.cjs) — [LAYERS.md](./LAYERS.md) |
...
| 21 | `git ls-files -z .splitbrief/ .nuke/ \| while IFS= read -r -d '' path; do [ -e "$path" ] && printf '%s\n' "$path"; done` | 0 matches | No present tracked runtime artifacts under `.splitbrief/` or `.nuke/` except intentional fixtures. |
...
| 24 | `rg -n "child_process\|editor-handover\|\bEDITOR\b\|\bVISUAL\b\|scratch" src/features/editor/ src/stores/ui/editor.ts src/app/overlays/editor.tsx --glob '!**/*.test.ts' --glob '!**/*.test.tsx'` | 0 matches | CON-C: the inline editor path spawns no external editor — no `child_process`, no editor handover, no `$EDITOR`/`$VISUAL`, no scratch-file writer. |
| 25 | `rg -n "\breadFileSync\b\|\breadFile\b\|from 'node:fs'\|from 'fs'" src/features/editor/ src/stores/ui/editor.ts src/app/overlays/editor.tsx --glob '!**/*.test.ts' --glob '!**/*.test.tsx'` | 0 matches | CON-E: the inline editor reads session artifacts only through `readSessionFileConfined`, never raw `fs`. |
| 26 | `rg -n "Ctrl\+\|Shift\+\|\bESC\b\|\bEsc\b\|⌃\|PgUp\|PgDn" src/app src/features src/components src/core/keybindings src/core/settings src/core/runtime/commands -g '!**/*.test.*' \| rg -v ':\s*(//\|\*\|/\*)' \| rg -v '^src/core/keybindings/normalize\.ts:'` | 0 matches | TUI key-token notation: rendered TUI sources use lowercase key tokens only (`ctrl+g`, `esc`, `pgup`), never `Ctrl+`/`Esc`/`⌃`/`PgUp`. Comments and the normalizer are exempt. |
| 27 | `tsx scripts/check-brand.ts >/dev/null && printf '0\n'` | 0 | Every maintained path and byte uses the canonical SPLITBRIEF identity; marked negative-contract literals are narrow and validated. |
| 28 | `{ rg -n "cli-tools\\.js\|CLI_TOOLS\|clampPromptForArgv" src testing \|\| true; test -e src/engine/runners/cli-tools.ts && printf '1\n' \|\| true; } \| wc -l` | 0 | Zero legacy `CLI_TOOLS` / `cli-tools.ts` / `clampPromptForArgv` — role registries replaced the monolithic CLI-tools module. |
| 29 | Node one-liner comparing `CLI_PLANNER_ADAPTERS`, `CLI_IMPLEMENTER_ADAPTERS`, and `KNOWN_PROVIDERS` keys to catalog tuples | 0 | Exhaustive CLI and API role registries match catalog tuples — no drift between admission catalogs and runtime registries. |
| 30 | `rg -ln "from 'react'\|from 'react-dom'\|from 'ink'" src/engine/ --glob '!**/*.test.ts' --glob '!**/*.test.tsx'` | 0 | Engine must not import `react`, `react-dom`, or `ink` — [LAYERS.md](./LAYERS.md) |
| 31 | `rg -i "\b(cursor\|antigravity\|mimo-token-plan\|mistral\|gemini\|cerebras\|zai\|minimax\|moonshot\|dashscope\|llama-cpp)\b" src/features/runners src/app/overlays/runners.tsx src/features/crew src/features/settings src/app/overlays/settings.tsx src/app/screens/setup.tsx --glob '*.tsx' --glob '!**/*.test.tsx'` | 0 | No vendor-ID JSX branches in seat-selection surfaces — the `runners`, `crew` and `settings` features plus the runners and settings overlay pages and the first-run setup screen, any casing (Crew's seat rows render inside Settings since T-058, so the standalone Crew overlay page is gone and the settings surfaces took its place). Admission verdicts drive registry wiring, not UI conditionals; word boundaries keep TUI `isCursor` terminology out of the match. |
| 32 | Ripgrep over `src/engine/providers/candidates`, `llama-cpp.ts`, and non-`openai-stream` `chat.completions.create` callers | 0 | One OpenAI-compatible stream transport (`openai-stream` only) — no duplicate fetch/stream parsers in provider modules. |
| 33 | `fetch(Json)?WithTimeout` + `privacyURL`/`termsURL` ripgrep with allow-lists | 0 | No runtime commercial metadata URL fetch outside `models.dev` module — pricing/terms/privacy URLs stay in catalog metadata, not live fetches. |
| 34 | Node one-liner checking `EXCLUDED_CLI_TOOL_IDS` and `FORBIDDEN_API_PROVIDER_IDS` against admitted runtime tuples | 0 | Zero excluded first-class CLI/API IDs in runtime tuples or catalogs — OMIT/FORBIDDEN verdicts stay out of production registries. |
| 35 | `find src -name 'index.ts' \| wc -l` | 0 | Zero source `index.ts` barrels — duplicate of gate 1 kept as a standalone count gate in the runner. |
| 36 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule direct-state \| wc -l` | 0 | T-065 direct state writes stay behind persistence/state-ops. |
| 37 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule raw-state-writer \| wc -l` | 0 | T-065 raw `state.json` replacement stays in core state persistence. |
| 38 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule reducer-import \| wc -l` | 0 | T-065 recovery reducer is imported only by its controller. |
| 39 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule briefs-ready \| wc -l` | 0 | T-065 `BRIEFS_READY` ownership is path-aware. |
| 40 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule quality-gate \| wc -l` | 0 | T-065 quality evaluation is controller-owned. |
| 41 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule continuation \| wc -l` | 0 | T-065 continuation guard has one implementation. |
| 42 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule recovery-provider \| wc -l` | 0 | T-065 recovery `planner.review` has one provider boundary. |
| 43 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule recovery-marker \| wc -l` | 0 | T-065 recovery continuation markers cannot re-enter review. |
| 44 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule adapters \| wc -l` | 0 | T-065 client adapters use controller/projection seams. |
| 45 | `tsx scripts/check-invariants.ts --t065-scan src/ --rule hydration \| wc -l` | 0 | T-065 hydration paths are exhaustively classified. |
| 46 | `rg -n "maxWidth=" src/app src/features src/components/overlays`, `rg -n "getResponsivePanelWidth\|getClampedTerminalWidth\|PICKER_WIDTHS\|SUB_PANEL_MAX_WIDTH\|MAX_PANEL_WIDTH\|SETUP_PANEL_WIDTH\|PALETTE_MAX_WIDTH" src`, and `rg -n "isSmall" src/app/overlays src/app/screens/home.tsx src/app/screens/setup.tsx src/features/home/layout.ts src/features/runners src/features/start-preparation src/features/workflow/cost-drilldown src/components/overlays src/components/pickers testing/visual/locators/registry.ts --glob '!**/*.test.ts' --glob '!**/*.test.tsx'`, summed | 0 | Pinned overlay widths are gone — panels size from the terminal, never from a checked-in constant or a `maxWidth` prop, and the retired width helpers stay retired. The `isSmall` path list names only the files whose `isSmall` was a **width** choice; the content-density branches that survive (`src/features/settings/mode-selector.tsx`, `src/features/home/components/recent-sessions-shell.tsx`, `src/features/summary/*`, `src/features/workflow/components/header.tsx`) are deliberately outside it. The `isSmall` clause excludes test files because `terminalSizeStore.__testReset({ isSmall })` is a legitimate test-only usage. |
| 47 | `rg -n "from '.*engine/providers/capability-inference" src/features src/app src/components src/core --glob '!**/*.test.ts' --glob '!**/*.test.tsx'` | 0 | Capability inference the UI needs lives in `src/core/`; `src/features`, `src/app`, `src/components` and `src/core` never import it from `src/engine/`. Layer direction is one-way — [LAYERS.md](./LAYERS.md) |

Gates are consolidated here; full rationale for each lives in the linked doc. The runner registers ids up to `'47'`; rows 36–45 are the T-065 controller-boundary scans, whose rule implementations live in `scripts/invariants/t065-scan.ts` (`T065_RULES`).
...
- [scripts/import-boundaries.ts](https://github.com/b4r7x/splitbrief/blob/main/scripts/import-boundaries.ts) — resolver backing gates 9 and 12c (cross-feature, components→features, engine→hooks/components/cli, layer-rank direction, stores→engine type-only channel)
- [.dependency-cruiser.cjs](https://github.com/b4r7x/splitbrief/blob/main/.dependency-cruiser.cjs) — dependency-cruiser config backing gate 19 (no runtime circular + LAYERS.md forbidden layer edges)
