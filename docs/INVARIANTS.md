...
| 31 | `rg -i "\b(cursor\|antigravity\|mimo-token-plan\|mistral\|gemini\|cerebras\|zai\|minimax\|moonshot\|dashscope\|llama-cpp)\b" src/features/runners src/app/overlays/runners.tsx src/features/crew src/app/overlays/crew.tsx src/app/screens/setup.tsx --glob '*.tsx' --glob '!**/*.test.tsx'` | 0 | No vendor-ID JSX branches in seat-selection surfaces — the `runners` and `crew` features plus their overlay pages and the first-run setup screen, any casing. Admission verdicts drive registry wiring, not UI conditionals; word boundaries keep TUI `isCursor` terminology out of the match. |
