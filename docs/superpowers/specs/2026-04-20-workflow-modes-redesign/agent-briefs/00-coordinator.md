...
5. **Every new slash command is added to `src/core/slash-commands/catalog.ts`** with a test in the catalog test file. **Additionally, every new slash command must be documented in `docs/SLASH-COMMANDS.md`** in the same patch. Briefs 04 (`/approve`), 05 (`/effort`), and 06 (`/attach`, `/detach`) each add slash commands and MUST update that doc.
