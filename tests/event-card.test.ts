...
// Helper: call renderEvent (pure function, no hooks) to inspect the returned React element tree
async function renderCard(event: TuiEvent, diffExpanded?: boolean) {
  const { renderEvent } = await import('../src/ui/event-card.js');
  return renderEvent(event, theme, diffExpanded);
}
...
describe('EventCard', () => {
  ...
  describe('planner-status', () => {
    it('running shows phase with ellipsis and accent color', async () => {
      ...
      const plannerText = findText(el, (p) => p.color === theme.planner);
      assert.ok(plannerText, 'should have planner-colored text');
    });
    ...
    it('done with summary shows muted summary text', async () => {
      ...
      const mutedText = findText(el, (p) => p.color === theme.textDim);
  ...
  describe('task-skipped', () => {
    it('renders muted text with reason', async () => {
      ...
      assert.ok(text.includes('dep failed'), `got: ${text}`);
      const mutedText = findText(el, (p) => p.color === theme.textDim);
  ...
  describe('validate', () => {
    it('passed shows success checkmark and duration', async () => {
      ...
      const text = collectText(el);
      assert.ok(text.includes('validator'), `got: ${text}`);
      assert.ok(text.includes('4.0s'), `got: ${text}`);
      assert.ok(text.includes('✓'), `should contain checkmarks, got: ${text}`);
    });
  ...
  describe('retry', () => {
    it('shows attempt count in warning color', async () => {
      ...
      const text = collectText(el);
      assert.ok(text.includes('retry') && text.includes('attempt 2/3'), `got: ${text}`);
  ...
  describe('escalate', () => {
    it('tier 1 shows warning bold', async () => {
      ...
      const text = collectText(el);
      assert.ok(text.includes('escalate') && text.includes('tier 1'), `got: ${text}`);
    ...
    it('tier 2 with hint shows hint text in muted color', async () => {
      ...
      const text = collectText(el);
      assert.ok(text.includes('escalate') && text.includes('tier 2'), `got: ${text}`);
      assert.ok(text.includes('Use existing middleware'), `got: ${text}`);
      const mutedHint = findText(el, (p) => p.color === theme.textDim && collectText(p.children).includes('middleware'));
  ...
  describe('git-commit', () => {
    it('renders message in muted color', async () => {
      ...
      const text = collectText(el);
      assert.ok(text.includes('committed') && text.includes('feat: add auth'), `got: ${text}`);
      const mutedText = findText(el, (p) => p.color === theme.textDim);
      assert.ok(mutedText, 'should use muted text color');
    });
  });
  ...
  describe('error', () => {
    it('renders error color bold with message', async () => {
      ...
      const errorText = findText(el, (p) => p.color === theme.error);
      assert.ok(errorText, 'should use error color');
    });
  });
  ...
  it('covers all TuiEvent types', async () => {
    ...
    for (const type of allTypes) {
      let event: TuiEvent;
      switch (type) {
        case 'planner-status': event = { type, ts: 1, phase: 'idle', status: 'running' }; break;
