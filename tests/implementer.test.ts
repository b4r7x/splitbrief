import { describe, it, afterEach } from 'node:test';
import assert from 'node:assert/strict';
import { mkdtempSync, writeFileSync, readFileSync, rmSync, existsSync, mkdirSync } from 'node:fs';
import { join } from 'node:path';
import { tmpdir } from 'node:os';
import { validateTaskPath } from '../src/utils/fs.js';
import { applyCode } from '../src/engine/apply.js';
import { makeTask as makeBaseTask } from './helpers/fixtures.js';
import type { Task } from '../src/types.js';

function makeTask(overrides: Partial<Task> = {}): Task {
  return makeBaseTask({ title: 'Test task', file: 'src/test.ts', description: 'test', ...overrides });
}

describe('validateTaskPath', () => {
  const projectDir = '/tmp/test-project';

  it('accepts a valid relative path', () => {
    const result = validateTaskPath(projectDir, 'src/valid.ts');
    assert.equal(result, '/tmp/test-project/src/valid.ts');
  });

  it('rejects path traversal with ../../', () => {
    assert.throws(
      () => validateTaskPath(projectDir, '../../outside'),
      { message: "Path '../../outside' escapes project directory" },
    );
  });

  it('rejects absolute paths', () => {
    assert.throws(
      () => validateTaskPath(projectDir, '/etc/passwd'),
      { message: "Path '/etc/passwd' escapes project directory" },
    );
  });
});

describe('applyCode', () => {
  let tempDir: string;

  afterEach(() => {
    if (tempDir && existsSync(tempDir)) {
      rmSync(tempDir, { recursive: true, force: true });
    }
  });

  it('create action writes a new file', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'impl-test-'));
    const task = makeTask({ action: 'create', file: 'src/new.ts' });
    const code = 'export const x = 1;\n';

    const result = applyCode(code, task, tempDir);

    assert.equal(result.success, true);
    assert.equal(readFileSync(join(tempDir, 'src', 'new.ts'), 'utf-8'), code);
  });

  it('modify action with file <200 lines does whole-file replacement', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'impl-test-'));
    const filePath = join(tempDir, 'src', 'existing.ts');
    const task = makeTask({ action: 'modify', file: 'src/existing.ts' });

    mkdirSync(join(tempDir, 'src'), { recursive: true });
    writeFileSync(filePath, 'export const old = true;\n');

    const newCode = 'export const updated = true;\n';
    const result = applyCode(newCode, task, tempDir);

    assert.equal(result.success, true);
    assert.equal(readFileSync(filePath, 'utf-8'), newCode);
  });

  it('modify action with search/replace markers applies patch', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'impl-test-'));
    const filePath = join(tempDir, 'src', 'large.ts');
    const task = makeTask({ action: 'modify', file: 'src/large.ts' });

    mkdirSync(join(tempDir, 'src'), { recursive: true });
    const lines = Array.from({ length: 250 }, (_, i) => `// line ${i + 1}`);
    lines[10] = 'export const old = true;';
    writeFileSync(filePath, lines.join('\n'));

    const patchCode = '<<<<<<< SEARCH\nexport const old = true;\n=======\nexport const patched = true;\n>>>>>>> REPLACE';
    const result = applyCode(patchCode, task, tempDir);

    assert.equal(result.success, true);
    const content = readFileSync(filePath, 'utf-8');
    assert.ok(content.includes('export const patched = true;'));
    assert.ok(!content.includes('export const old = true;'));
  });

  it('search/replace returns error when search block not found', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'impl-test-'));
    const filePath = join(tempDir, 'src', 'large2.ts');
    const task = makeTask({ action: 'modify', file: 'src/large2.ts' });

    mkdirSync(join(tempDir, 'src'), { recursive: true });
    const lines = Array.from({ length: 250 }, (_, i) => `// line ${i + 1}`);
    writeFileSync(filePath, lines.join('\n'));

    const patchCode = '<<<<<<< SEARCH\nthis text does not exist in the file\n=======\nreplacement\n>>>>>>> REPLACE';
    const result = applyCode(patchCode, task, tempDir);

    assert.equal(result.success, false);
    assert.ok(result.error?.includes('Search block not found'));
  });

  it('search/replace preserves dollar sign patterns verbatim', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'impl-test-'));
    const filePath = join(tempDir, 'src', 'dollar.ts');
    const task = makeTask({ action: 'modify', file: 'src/dollar.ts' });

    mkdirSync(join(tempDir, 'src'), { recursive: true });
    const lines = Array.from({ length: 250 }, (_, i) => `// line ${i + 1}`);
    lines[5] = 'const a = "old1";';
    lines[10] = 'const b = "old2";';
    lines[15] = 'const c = "old3";';
    lines[20] = 'const d = "old4";';
    writeFileSync(filePath, lines.join('\n'));

    const patchCode = [
      '<<<<<<< SEARCH',
      'const a = "old1";',
      '=======',
      'const a = `Hello ${variable}`;',
      '>>>>>>> REPLACE',
      '<<<<<<< SEARCH',
      'const b = "old2";',
      '=======',
      'const b = "$1 captured group";',
      '>>>>>>> REPLACE',
      '<<<<<<< SEARCH',
      'const c = "old3";',
      '=======',
      'const c = "$& matched text";',
      '>>>>>>> REPLACE',
      '<<<<<<< SEARCH',
      'const d = "old4";',
      '=======',
      'const d = "$$ escaped dollar";',
      '>>>>>>> REPLACE',
    ].join('\n');

    const result = applyCode(patchCode, task, tempDir);

    assert.equal(result.success, true);
    const content = readFileSync(filePath, 'utf-8');
    assert.ok(content.includes('const a = `Hello ${variable}`;'));
    assert.ok(content.includes('const b = "$1 captured group";'));
    assert.ok(content.includes('const c = "$& matched text";'));
    assert.ok(content.includes('const d = "$$ escaped dollar";'));
  });

  it('rejects path traversal in task file', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'impl-test-'));
    const task = makeTask({ action: 'create', file: '../../etc/evil.ts' });
    const result = applyCode('malicious code', task, tempDir);
    assert.equal(result.success, false);
    assert.ok(result.error?.includes('escapes project directory'));
  });
});
